...
| Doc | Role |
|---|---|
| [`../../bench/dual/README.md`](../../bench/dual/README.md) | the **runbook** — prerequisites, the three entry points, env knobs, reading the board, machine hygiene, troubleshooting. |
| [`../../bench/dual/BENCH_PROTOCOL.md`](../../bench/dual/BENCH_PROTOCOL.md) | the **rationale** — why one process / interleaved / min-over-reps, and the pass/fail bars. |

They live beside the scripts (`bench/dual/`) so the harness and its docs stay in
...
```bash
bench/dual/update_routine.sh e etbsv       # re-time one routine + refresh the .md
bench/dual/run_dual.sh q qsyrk,qsyr2k      # sweep a family → console board
cmake --workflow --preset sweep            # CI-style: run_dual.sh per family via ctest
```

`update_routine.sh` re-renders the committed `reports/dual_scoreboard.md`; raw
