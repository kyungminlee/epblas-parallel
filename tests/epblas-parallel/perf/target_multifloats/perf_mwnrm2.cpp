 ...
 *
 * Kernel-isolated C perf harness for mwnrm2 (overlay vs migrated).
...
BLAS_EXTERN MFR mwnrm2_(const int *, const MFC *, const int *);
BLAS_EXTERN MFR mwnrm2_migrated_(const int *, const MFC *, const int *);
...
static void run_one(int N, int iters, int warmup) {
    ...
    for (int r2 = 0; r2 < warmup; ++r2) {
        r = mwnrm2_(&N, X, &one);
        r = mwnrm2_migrated_(&N, X, &one);
    }
    double t_subject, t_mg;
    PERF_TIME(t_subject, iters, r = mwnrm2_(&N, X, &one));
    PERF_TIME(t_mg,      iters, r = mwnrm2_migrated_(&N, X, &one));
    double flops = 4.0 * (double)N;
    PERF_EMIT("mwnrm2", "-", N, iters, flops, t_subject, t_mg);
