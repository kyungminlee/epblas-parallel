#!/usr/bin/env python3
"""Aggregate the focused task-#96 5-way sweep into a per-routine/size table.

min-over-reps for each of the 4 subject columns (ep omp1/omp4, par omp1/omp4)
plus the shared migrated reference. Reports bare wall time (ns/call) and the
key ratios; flags regressions (par slower than the ep openblas reference, or
par slower than migrated serial, by >5%).
"""
import csv
from collections import defaultdict
from pathlib import Path

RAW = Path(__file__).parent / "cmp5_task96_raw.tsv"
RUNS = ["epopenblas-omp1", "epopenblas-omp4", "parallel-blas-omp1", "parallel-blas-omp4"]

# (routine,size,run_id) -> min subject_ns ; (routine,size) -> min migrated_ns
...
REG = 1.05  # par slower than baseline by >5% => flag
worst = {}
