...
def precision_of(routine):
    """Map a routine name to its element-precision family.

    The leading letter encodes the element type (e/y=kind10, q/x=kind16,
    m/w=multifloats); the `i*amax` argmax routines carry it in the second
    letter (ieamax, iqamax, imamax, …).
    """
    p = routine[1] if routine[0] == "i" else routine[0]
    return {"e": "kind10", "y": "kind10",
            "q": "kind16", "x": "kind16",
            "m": "multifloats", "w": "multifloats"}.get(p, "kind16")
...
def main():
    ...
    out = []
    precisions = sorted({precision_of(r["routine"]) for r in rows})
    out.append(f"# 5-way comparison: {EP1.label.split('-')[0]} / {P1.label.split('-')[0]} / {MIG_LABEL.split('-')[0]} — {' + '.join(precisions)} (e/y = REAL/COMPLEX(KIND=10), q/x = REAL/COMPLEX(KIND=16), m/w = REAL/COMPLEX double-double (multifloats))")
    out.append("")
    ...
    out.append("- All four overlay binaries link the SAME `tests/epblas-parallel/perf/target_<kind>/perf_<r>.c` source (kind10 for e/y routines, kind16 for q/x, multifloats for m/w) — only the C-overlay symbol differs.")
