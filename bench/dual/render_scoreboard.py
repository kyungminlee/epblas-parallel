...
The one-routine cycle is bench/dual/update_routine.sh.

Usage:
    render_scoreboard.py [out.md] [results_dir ...]
        out.md        default: reports/dual_scoreboard.md
...
def main():
    out_path = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / "reports/dual_scoreboard.md"
    if len(sys.argv) > 2:
    ...
    out.append(f"_Generated {stamp} by `bench/dual/render_scoreboard.py`._\n")
    out.append("All values are **bare wall time (ns/call)**, ratio = **par / reference, "
               ...
               "OpenBLAS clone). See `bench/dual/BENCH_PROTOCOL.md`.\n")
    # headline table
