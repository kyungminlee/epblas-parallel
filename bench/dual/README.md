...
```bash
bench/dual/update_routine.sh e etbsv          # one routine
bench/dual/update_routine.sh q qtrmm,qtrmv    # a few
```

This rebuilds the archives, re-times just those routines into the full-surface
`results_<fam>` dir, and re-renders `reports/dual_scoreboard.md` from **all**
...
```bash
bench/dual/run_dual.sh e                       # all 75 kind10 routines
bench/dual/run_dual.sh q qsyrk,qsyr2k          # just these two
```
...
```bash
bench/dual/render_scoreboard.py                # -> reports/dual_scoreboard.md
```
...
```bash
REPS=20 bench/dual/run_dual.sh e enrm2
```
...
- **Committed** → only `reports/dual_scoreboard.md`. Never commit anything under
  `bench/` beyond the scripts, and never the raw `.txt`.
...
| `render_scoreboard.py` | pure reduction of `results_{m,e,q}/` → `reports/dual_scoreboard.md`. |
...
| `../../scripts/gen_dual_harnesses.py` | emit the per-routine dual drivers (`--family`, `--routines`, `--list`). |
