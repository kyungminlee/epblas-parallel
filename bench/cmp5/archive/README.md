...
live under `bench/dual/`:

- `bench/dual/run_dual.sh`   — end-to-end sweep (build → namespace → generate → run → aggregate)
- `bench/dual/agg_dual.py`   — scoreboard (par1 ≤ min(ob1,mig1); par4 ≤ ob4; par4/par1 scaling)
- `bench/dual/BENCH_PROTOCOL.md` — the current protocol
