/*
 * qblas_l3_real.h — shared L3 kernel + packers for the kind16 real path.
 *
 ...
 *
 * Shared across the L3 real ports (qgemm, qsymm, qsyrk, qsyr2k, qtrmm,
 * qtrsm, qgemmtr). Each routine still owns its own dispatch + level3
 ...
 * 128-bit __float128, so the scalar generic kernel is the right
 * starting point and `gcc -O3` keeps the four accumulators on the
 * 8-deep x87 register stack across the K-loop.
 */
#ifndef QBLAS_L3_REAL_H
#define QBLAS_L3_REAL_H
...
#define QBLAS_EGEMM_MR 2
#define QBLAS_EGEMM_NR 2
 ...
 *
 * Defaults sized so MC*KC of `__float128` (16 B each on x86-64)
 ...
 * 128-bit-no-SIMD case — they don't trace back to any specific
 * other port. */
#define QBLAS_EGEMM_GEMM_P  64
#define QBLAS_EGEMM_GEMM_Q  256
#define QBLAS_EGEMM_GEMM_R  512
 ...
 * of qblas_egemm_ncopy — handles the trans cases of A and B.
 */
 ...
 * pattern (QBLAS_MC, QBLAS_KC, QBLAS_NC) so the same tuning env
 ...
 *
 * Port source: OpenBLAS kernel/generic/symm_ucopy_2.c (qsymm_ucopy)
 *              and symm_lcopy_2.c (qsymm_lcopy).
 *
 ...
 * use qblas_egemm_beta (which touches the full M×N rectangle).
 *
 * Mirrors OpenBLAS's `syrk_beta` (driver/level3/syrk_k.c lines 52–99).
 */
void qblas_esyrk_beta_u(ptrdiff_t n,
                        __float128 beta,
                        __float128 *c, ptrdiff_t ldc);
void qblas_esyrk_beta_l(ptrdiff_t n,
                        __float128 beta,
                        __float128 *c, ptrdiff_t ldc);
 ...
 *   - Entirely in the kept triangle → full qblas_egemm_kernel call.
 ...
 *   macro: if non-zero, the on-diagonal element is forced to 1.0Q and
 ...
 * qblas_egemm_kernel — TRMM's L3 nest fully overwrites each B tile
 ...
 *
 * Same as qblas_egemm_kernel but with C := alpha * Ap * Bp (no
 ...
 * Implemented as: zero the tile then call qblas_egemm_kernel.
 */
 ...
 * diagonal-crossing pattern. unit==1 forces diag to 1.0Q (no inversion).
 *
 * **Critical structural difference vs the TRMM packers**: TRSM packers
 * write `1.0Q / a[diag]` (or 1.0Q when unit) at every on-diagonal
...
void qblas_etrsm_iutcopy(ptrdiff_t m, ptrdiff_t n,
                         const __float128 *a, ptrdiff_t lda,
                         ptrdiff_t offset, __float128 *b, int unit);
 ...
 *      qblas_egemm_kernel — note its += accumulate semantics line up
