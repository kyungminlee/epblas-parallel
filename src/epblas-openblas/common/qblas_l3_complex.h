/*
 * qblas_l3_complex.h — shared L3 kernel + packers for the kind16 complex path.
 *
 ...
 */
#ifndef QBLAS_L3_COMPLEX_H
#define QBLAS_L3_COMPLEX_H
 ...
 */
void qblas_ysyrk_beta_u(ptrdiff_t n,
                        __float128 beta_r, __float128 beta_i,
                        __float128 *c, ptrdiff_t ldc);
void qblas_ysyrk_beta_l(ptrdiff_t n,
                        __float128 beta_r, __float128 beta_i,
                        __float128 *c, ptrdiff_t ldc);
 ...
 * OpenBLAS driver/level3/zherk_beta.c. Differs from xsyrk_beta_{u,l}
 * by: (a) real beta only, no imag mix; (b) unconditional diag imag = 0.
 */
void qblas_yherk_beta_u(ptrdiff_t n, __float128 beta_r,
                        __float128 *c, ptrdiff_t ldc);
void qblas_yherk_beta_l(ptrdiff_t n, __float128 beta_r,
                        __float128 *c, ptrdiff_t ldc);

/* ── HERK kernel: SYRK kernel with diagonal-imag clamp ───────────────
 *
 * Port of OpenBLAS driver/level3/zherk_kernel.c. Identical to
 * qblas_ysyrk_kernel_{u,l} *except* that the diagonal element of each
 ...
 * qblas_ysyr2k_kernel_{u,l}; differs only in the diagonal NR×NR sub-
 * block writeback:
 *
 ...
 * the conjugation via the packer `conj` flag — see xher2k.c for the
 ...
 * added to absorb conjugation at pack time (matching xgemm/xsymm
...
void qblas_ytrmm_iutcopy(ptrdiff_t m, ptrdiff_t n,
                         const __float128 *a, ptrdiff_t lda,
                         ptrdiff_t posX, ptrdiff_t posY,
                         __float128 *b, int unit, int conj);
 ...
 * qblas_ygemm_kernel). LEFT and TRANSA macros converted to runtime
 * `left` and `trans` flags. C := alpha * ba * bb (overwrite), per-
 * element 2 __float128s (interleaved re,im).
 */
 ...
 * complex tile, then call qblas_ygemm_kernel.
 */
 ...
 * lda is in COMPLEX(KIND=16) elements; the implementation doubles it
 ...
 * is sign-flipped (matches the xgemm/xtrmm pattern of absorbing
 ...
 *
 * Per-element width = 2 __float128s (interleaved re,im); ldc, k, m,
