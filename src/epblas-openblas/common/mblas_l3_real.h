/*
 * mblas_l3_real.h — shared L3 kernel + packers for the kind10 real path.
 *
 ...
 * 80-bit multifloats::float64x2, so the scalar generic kernel is the right
 * starting point and `gcc -O3` keeps the four accumulators on the
 * 8-deep x87 register stack across the K-loop.
 */
#ifndef MBLAS_L3_REAL_H
#define MBLAS_L3_REAL_H
...
#define MBLAS_EGEMM_MR 2
#define MBLAS_EGEMM_NR 2
 ...
 *
 * Defaults sized so MC*KC of `multifloats::float64x2` (16 B each on x86-64)
 ...
 * other port. */
#define MBLAS_EGEMM_GEMM_P  64
#define MBLAS_EGEMM_GEMM_Q  256
#define MBLAS_EGEMM_GEMM_R  512
 ...
 * of mblas_egemm_ncopy — handles the trans cases of A and B.
 */
 ...
 * use mblas_egemm_beta (which touches the full M×N rectangle).
 *
 * Mirrors OpenBLAS's `syrk_beta` (driver/level3/syrk_k.c lines 52–99).
 */
void mblas_esyrk_beta_u(ptrdiff_t n,
                        multifloats::float64x2 beta,
                        multifloats::float64x2 *c, ptrdiff_t ldc);
void mblas_esyrk_beta_l(ptrdiff_t n,
                        multifloats::float64x2 beta,
                        multifloats::float64x2 *c, ptrdiff_t ldc);
 ...
 *   - Entirely in the kept triangle → full mblas_egemm_kernel call.
...
void mblas_etrmm_iutcopy(ptrdiff_t m, ptrdiff_t n,
                         const multifloats::float64x2 *a, ptrdiff_t lda,
                         ptrdiff_t posX, ptrdiff_t posY,
                         multifloats::float64x2 *b, int unit);
 ...
 * mblas_egemm_kernel — TRMM's L3 nest fully overwrites each B tile
 ...
 *
 * Same as mblas_egemm_kernel but with C := alpha * Ap * Bp (no
 ...
 * Implemented as: zero the tile then call mblas_egemm_kernel.
 */
...
void mblas_etrsm_iutcopy(ptrdiff_t m, ptrdiff_t n,
                         const multifloats::float64x2 *a, ptrdiff_t lda,
                         ptrdiff_t offset, multifloats::float64x2 *b, int unit);
 ...
 *      mblas_egemm_kernel — note its += accumulate semantics line up
