/*
 * mblas_l3_complex.c — shared L3 microkernel + packers (kind10 complex).
 *
 * Port source: OpenBLAS.
 *   - kernel/generic/zgemmkernel_2x2.c   ← mblas_ygemm_kernel
 *                                          (NN path only; conjugation absorbed
 *                                           into the packers via the `conj` flag)
 *   - kernel/generic/zgemm_ncopy_2.c     ← mblas_ygemm_ncopy
 *   - kernel/generic/zgemm_tcopy_2.c     ← mblas_ygemm_tcopy
 *   - kernel/generic/zgemm_beta.c        ← mblas_ygemm_beta
 */
...
#define MR MBLAS_YGEMM_MR
#define NR MBLAS_YGEMM_NR
 ...
 */
void mblas_ygemm_kernel(ptrdiff_t bm, ptrdiff_t bn, ptrdiff_t bk,
                        T alphar, T alphai,
                        const T *Ap,
                        const T *Bp,
                        T *C, ptrdiff_t ldc)
 ...
 */
void mblas_ygemm_ncopy(ptrdiff_t m, ptrdiff_t n,
                       int conj,
                       const T *a, ptrdiff_t lda,
                       T *b)
{
    const T sign = conj ? -1.0 : 1.0;
    const T *a_off = a;
    T *b_off = b;
...
void mblas_ygemm_tcopy(ptrdiff_t m, ptrdiff_t n,
                       int conj,
                       const T *a, ptrdiff_t lda,
                       T *b)
{
...
void mblas_ygemm_beta(ptrdiff_t m, ptrdiff_t n,
                      T beta_r, T beta_i,
                      T *c, ptrdiff_t ldc)
{
    const ptrdiff_t ldc2 = ldc * 2;

    if (beta_r == 1.0 && beta_i == 0.0) return;
...
void mblas_ygemm_blocks(int *mc, int *kc, int *nc) {
    if (!g_mc) {
        g_mc = env_int("MBLAS_MC", MBLAS_YGEMM_GEMM_P);
        g_kc = env_int("MBLAS_KC", MBLAS_YGEMM_GEMM_Q);
        g_nc = env_int("MBLAS_NC", MBLAS_YGEMM_GEMM_R);
    }
 ...
 */
void mblas_ysymm_ucopy(ptrdiff_t m, ptrdiff_t n,
                       const T *a, ptrdiff_t lda,
...
void mblas_ysymm_lcopy(ptrdiff_t m, ptrdiff_t n,
                       const T *a, ptrdiff_t lda,
                       ptrdiff_t posX, ptrdiff_t posY,
                       T *b)
{
 ...
 */
void mblas_yhemm_ucopy(ptrdiff_t m, ptrdiff_t n,
                       const T *a, ptrdiff_t lda,
                       ptrdiff_t posX, ptrdiff_t posY,
                       T *b)
{
...
void mblas_yhemm_lcopy(ptrdiff_t m, ptrdiff_t n,
                       const T *a, ptrdiff_t lda,
                       ptrdiff_t posX, ptrdiff_t posY,
                       T *b)
{
 ...
 * call site triggers.
 *
 * See the header comment on mblas_yhemm_ucopy_oc for the rationale
 * (why we need a distinct OC variant instead of reusing the IC one as
 * upstream OpenBLAS does).
 */
void mblas_yhemm_ucopy_oc(ptrdiff_t m, ptrdiff_t n,
                          const T *a, ptrdiff_t lda,
                          ptrdiff_t posX, ptrdiff_t posY,
                          T *b)
{
...
void mblas_yhemm_lcopy_oc(ptrdiff_t m, ptrdiff_t n,
                          const T *a, ptrdiff_t lda,
                          ptrdiff_t posX, ptrdiff_t posY,
                          T *b)
{
 ...
 * inside the row loop instead of touching `mblas_ygemm_beta`'s full-
 * rectangle helper.
 */
...
void mblas_ysyrk_beta_u(ptrdiff_t n, T br, T bi, T *c, ptrdiff_t ldc) {
    if (br == 1.0 && bi == 0.0) return;
    const ptrdiff_t ldc2 = 2 * ldc;
    for (ptrdiff_t j = 0; j < n; ++j) {
...
void mblas_ysyrk_beta_l(ptrdiff_t n, T br, T bi, T *c, ptrdiff_t ldc) {
    if (br == 1.0 && bi == 0.0) return;
    const ptrdiff_t ldc2 = 2 * ldc;
    for (ptrdiff_t j = 0; j < n; ++j) {
 ...
 * complex element.
 */
void mblas_ysyrk_kernel_u(ptrdiff_t m, ptrdiff_t n, ptrdiff_t k,
                          T alphar, T alphai,
                          const T *a, const T *b,
                          T *c, ptrdiff_t ldc, ptrdiff_t offset)
{
    ...
    if (m + offset < 0) {
        mblas_ygemm_kernel(m, n, k, alphar, alphai, a, b, c, ldc);
        return;
    }
    if (n < offset) {
        return;
    ...
    if (n > m + offset) {
        mblas_ygemm_kernel(m, n - m - offset, k, alphar, alphai,
                           a, b + (m + offset) * k * 2,
                           c + (m + offset) * ldc2, ldc);
        n = m + offset;
        if (n <= 0) return;
    ...
    if (offset < 0) {
        mblas_ygemm_kernel(-offset, n, k, alphar, alphai, a, b, c, ldc);
        a -= offset * k * 2;
        c -= offset * 2;
        m += offset;
        offset = 0;
    ...
    for (ptrdiff_t loop = 0; loop < n; loop += NR) {
        ...
        if (loop > 0) {
            mblas_ygemm_kernel(loop, nn, k, alphar, alphai,
                               a, b + loop * k * 2,
                               c + loop * ldc2, ldc);
        }

        for (ptrdiff_t z = 0; z < nn * nn * 2; ++z) subbuf[z] = 0;
        mblas_ygemm_kernel(nn, nn, k, alphar, alphai,
...
void mblas_ysyrk_kernel_l(ptrdiff_t m, ptrdiff_t n, ptrdiff_t k,
                          T alphar, T alphai,
                          const T *a, const T *b,
                          T *c, ptrdiff_t ldc, ptrdiff_t offset)
{
    ...
    if (offset > 0) {
        mblas_ygemm_kernel(m, offset, k, alphar, alphai, a, b, c, ldc);
        b += offset * k * 2;
        c += offset * ldc2;
        n -= offset;
        offset = 0;
    ...
    if (m > n - offset) {
        mblas_ygemm_kernel(m - n + offset, n, k, alphar, alphai,
                           a + (n - offset) * k * 2, b,
                           c + (n - offset) * 2, ldc);
        m = n + offset;
        if (m <= 0) return;
    ...
    for (ptrdiff_t loop = 0; loop < n; loop += NR) {
        ...
        if (m > mm + nn) {
            mblas_ygemm_kernel(m - mm - nn, nn, k, alphar, alphai,
 ...
 * on the diagonal, even when beta == 1.0 (matches zherk_beta).
 */
void mblas_yherk_beta_u(ptrdiff_t n, T br, T *c, ptrdiff_t ldc) {
    const ptrdiff_t ldc2 = 2 * ldc;
    for (ptrdiff_t j = 0; j < n; ++j) {
        T *col = c + j * ldc2;
        if (br == 0.0) {
...
void mblas_yherk_beta_l(ptrdiff_t n, T br, T *c, ptrdiff_t ldc) {
    const ptrdiff_t ldc2 = 2 * ldc;
    for (ptrdiff_t j = 0; j < n; ++j) {
        T *col = c + j * ldc2;
        if (br == 0.0) {
 ...
 * twin of mblas_ysyrk_kernel_{u,l}; differs only in the diagonal
 ...
 */
void mblas_yherk_kernel_u(ptrdiff_t m, ptrdiff_t n, ptrdiff_t k,
                          T alphar,
                          const T *a, const T *b,
                          T *c, ptrdiff_t ldc, ptrdiff_t offset)
{
    ...
    if (m + offset < 0) {
        mblas_ygemm_kernel(m, n, k, alphar, 0.0, a, b, c, ldc);
        return;
    }
    if (n < offset) {
        return;
    ...
    if (n > m + offset) {
        mblas_ygemm_kernel(m, n - m - offset, k, alphar, 0.0,
                           a, b + (m + offset) * k * 2,
                           c + (m + offset) * ldc2, ldc);
        n = m + offset;
        if (n <= 0) return;
    ...
    if (offset < 0) {
        mblas_ygemm_kernel(-offset, n, k, alphar, 0.0, a, b, c, ldc);
        a -= offset * k * 2;
        c -= offset * 2;
        m += offset;
        offset = 0;
    ...
    for (ptrdiff_t loop = 0; loop < n; loop += NR) {
        ...
        if (loop > 0) {
            mblas_ygemm_kernel(loop, nn, k, alphar, 0.0,
                               a, b + loop * k * 2,
                               c + loop * ldc2, ldc);
        }

        for (ptrdiff_t z = 0; z < nn * nn * 2; ++z) subbuf[z] = 0;
        mblas_ygemm_kernel(nn, nn, k, alphar, 0.0,
                           a + loop * k * 2, b + loop * k * 2,
                           subbuf, nn);
...
void mblas_yherk_kernel_l(ptrdiff_t m, ptrdiff_t n, ptrdiff_t k,
                          T alphar,
                          const T *a, const T *b,
                          T *c, ptrdiff_t ldc, ptrdiff_t offset)
{
    ...
    if (offset > 0) {
        mblas_ygemm_kernel(m, offset, k, alphar, 0.0, a, b, c, ldc);
        b += offset * k * 2;
        c += offset * ldc2;
        n -= offset;
        offset = 0;
    ...
    if (m > n - offset) {
        mblas_ygemm_kernel(m - n + offset, n, k, alphar, 0.0,
                           a + (n - offset) * k * 2, b,
                           c + (n - offset) * 2, ldc);
        m = n + offset;
        if (m <= 0) return;
    ...
    for (ptrdiff_t loop = 0; loop < n; loop += NR) {
        ...
        if (m > mm + nn) {
            mblas_ygemm_kernel(m - mm - nn, nn, k, alphar, 0.0,
                               a + (mm + nn) * k * 2, b + loop * k * 2,
                               c + (mm + nn) * 2 + loop * ldc2, ldc);
        }
    }
 ...
 */
void mblas_ysyr2k_kernel_u(ptrdiff_t m, ptrdiff_t n, ptrdiff_t k,
                           T alphar, T alphai,
                           const T *a, const T *b,
...
void mblas_ysyr2k_kernel_l(ptrdiff_t m, ptrdiff_t n, ptrdiff_t k,
                           T alphar, T alphai,
                           const T *a, const T *b,
                           T *c, ptrdiff_t ldc, ptrdiff_t offset, int flag)
{
 ...
 * twin of mblas_ysyr2k_kernel_{u,l}; differs only in the diagonal
 ...
 */
void mblas_yher2k_kernel_u(ptrdiff_t m, ptrdiff_t n, ptrdiff_t k,
                           T alphar, T alphai,
                           const T *a, const T *b,
                           T *c, ptrdiff_t ldc, ptrdiff_t offset, int flag)
{
...
void mblas_yher2k_kernel_l(ptrdiff_t m, ptrdiff_t n, ptrdiff_t k,
                           T alphar, T alphai,
                           const T *a, const T *b,
                           T *c, ptrdiff_t ldc, ptrdiff_t offset, int flag)
{
 ...
 *
 * Same as mblas_ygemm_kernel but C := alpha * Ap * Bp (overwrite).
 * Used by the TRMM L3 driver for off-diagonal sub-tiles.
 */
void mblas_ygemm_kernel_store(ptrdiff_t bm, ptrdiff_t bn, ptrdiff_t bk,
                              T alphar, T alphai,
                              const T *Ap,
                              const T *Bp,
                              T *C, ptrdiff_t ldc)
{
    ...
    mblas_ygemm_kernel(bm, bn, bk, alphar, alphai, Ap, Bp, C, ldc);
}
...
void mblas_ytrmm_iutcopy(ptrdiff_t m, ptrdiff_t n,
                         const T *a, ptrdiff_t lda,
                         ptrdiff_t posX, ptrdiff_t posY,
...
void mblas_ytrmm_iuncopy(ptrdiff_t m, ptrdiff_t n,
                         const T *a, ptrdiff_t lda,
                         ptrdiff_t posX, ptrdiff_t posY,
                         T *b, int unit, int conj)
{
...
void mblas_ytrmm_iltcopy(ptrdiff_t m, ptrdiff_t n,
                         const T *a, ptrdiff_t lda,
                         ptrdiff_t posX, ptrdiff_t posY,
                         T *b, int unit, int conj)
{
...
void mblas_ytrmm_ilncopy(ptrdiff_t m, ptrdiff_t n,
                         const T *a, ptrdiff_t lda,
                         ptrdiff_t posX, ptrdiff_t posY,
                         T *b, int unit, int conj)
{
...
static inline void compinv_ld(T *b, T ar, T ai, int unit) {
    ...
    T ratio, den;
    if (multifloats::fabs(ar) >= multifloats::fabs(ai)) {
        ratio = ai / ar;
        den   = T(1.0) / (ar * (T(1.0) + ratio * ratio));
        b[0] =  den;
        b[1] = -ratio * den;
    } else {
        ratio = ar / ai;
        den   = T(1.0) / (ai * (T(1.0) + ratio * ratio));
        b[0] =  ratio * den;
        b[1] = -den;
    }
}
...
void mblas_ytrsm_ilncopy(ptrdiff_t m, ptrdiff_t n,
                         const T *a, ptrdiff_t lda,
                         ptrdiff_t offset, T *b, int unit, int conj)
{
    ptrdiff_t i, ii, j, jj;
    T data01 = 0.0, data02 = 0.0, data03, data04;
    T data05, data06, data07 = 0.0, data08 = 0.0;
    const T *a1, *a2;
...
void mblas_ytrsm_iltcopy(ptrdiff_t m, ptrdiff_t n,
                         const T *a, ptrdiff_t lda,
                         ptrdiff_t offset, T *b, int unit, int conj)
{
    ptrdiff_t i, ii, j, jj;
...
void mblas_ytrsm_iuncopy(ptrdiff_t m, ptrdiff_t n,
                         const T *a, ptrdiff_t lda,
                         ptrdiff_t offset, T *b, int unit, int conj)
{
    ptrdiff_t i, ii, j, jj;
...
void mblas_ytrsm_iutcopy(ptrdiff_t m, ptrdiff_t n,
                         const T *a, ptrdiff_t lda,
                         ptrdiff_t offset, T *b, int unit, int conj)
{
    ptrdiff_t i, ii, j, jj;
...
void mblas_ytrsm_kernel(int left, int trans,
                        ptrdiff_t bm, ptrdiff_t bn, ptrdiff_t bk,
                        const T *ba, const T *bb,
                        T *C, ptrdiff_t ldc,
                        ptrdiff_t offset)
    ...
    if (left && !trans) {
        ...
        while (j > 0) {
            ...
            if (bm & (UR - 1)) {
                for (i = 1; i < UR; i *= 2) {
                    if (bm & i) {
                        ...
                        cc = C + ((bm & ~(i - 1)) - i) * 2;
                        if (bk - kk > 0) {
                            mblas_ygemm_kernel(i, UN, bk - kk, dm1r, dm1i,
                                               aa + i * kk * 2,
            ...
            if (i > 0) {
                ...
                do {
                    if (bk - kk > 0) {
                        mblas_ygemm_kernel(UR, UN, bk - kk, dm1r, dm1i,
                                           aa + UR * kk * 2,
                                           b_buf + UN * kk * 2, cc, ldc);
                    }
                    zsolve_LN(UR, UN,
            ...
            while (j > 0) {
                if (bn & j) {
                    ...
                    if (bm & (UR - 1)) {
                        for (i = 1; i < UR; i *= 2) {
                            if (bm & i) {
                                ...
                                cc = C + ((bm & ~(i - 1)) - i) * 2;
                                if (bk - kk > 0) {
                                    mblas_ygemm_kernel(i, j, bk - kk, dm1r, dm1i,
                                                       aa + i * kk * 2,
                                                       b_buf + j * kk * 2, cc, ldc);
                                }
                                zsolve_LN(i, j,
                    ...
                    if (i > 0) {
                        ...
                        do {
                            if (bk - kk > 0) {
                                mblas_ygemm_kernel(UR, j, bk - kk, dm1r, dm1i,
                                                   aa + UR * kk * 2,
                                                   b_buf + j * kk * 2, cc, ldc);
                            }
                            zsolve_LN(UR, j,
        ...
        while (j > 0) {
            ...
            while (i > 0) {
                if (kk > 0) {
                    mblas_ygemm_kernel(UR, UN, kk, dm1r, dm1i, aa, b_buf, cc, ldc);
                }
                zsolve_LT(UR, UN,
                          aa + kk * UR * 2,
                          b_buf + kk * UN * 2,
            ...
            if (bm & (UR - 1)) {
                ...
                while (i > 0) {
                    if (bm & i) {
                        if (kk > 0) {
                            mblas_ygemm_kernel(i, UN, kk, dm1r, dm1i, aa, b_buf, cc, ldc);
                        }
                        zsolve_LT(i, UN,
                                  aa + kk * i * 2,
                                  b_buf + kk * UN * 2,
        ...
        if (bn & (UN - 1)) {
            ...
            while (j > 0) {
                if (bn & j) {
                    ...
                    while (i > 0) {
                        if (kk > 0) {
                            mblas_ygemm_kernel(UR, j, kk, dm1r, dm1i, aa, b_buf, cc, ldc);
                        }
                        zsolve_LT(UR, j,
                                  aa + kk * UR * 2,
                                  b_buf + kk * j * 2,
                    ...
                    if (bm & (UR - 1)) {
                        ...
                        while (i > 0) {
                            if (bm & i) {
                                if (kk > 0) {
                                    mblas_ygemm_kernel(i, j, kk, dm1r, dm1i, aa, b_buf, cc, ldc);
                                }
                                zsolve_LT(i, j,
                                          aa + kk * i * 2,
                                          b_buf + kk * j * 2,
