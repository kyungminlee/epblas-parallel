/*
 * mblas_l3_complex.h — shared L3 kernel + packers for the kind10 complex path.
 *
 ...
 */
#ifndef MBLAS_L3_COMPLEX_H
#define MBLAS_L3_COMPLEX_H
...
#define MBLAS_YGEMM_MR 2
#define MBLAS_YGEMM_NR 2

/* ── Cache-block defaults (overridable via getenv at routine entry) ─
 *
 * Sized so MC*KC of complex `multifloats::float64x2` (32 B each on x86-64) fits
...
#define MBLAS_YGEMM_GEMM_P  64
#define MBLAS_YGEMM_GEMM_Q  256
#define MBLAS_YGEMM_GEMM_R  512

/* ── Microkernel: NN-only 2x2 complex outer-product over K ────────────
 *
 * Compute C += alpha * Ap * Bp for one (bm, bn) tile of C, all dims in
 * complex elements. ldc is in complex elements; C is `multifloats::float64x2 *`
 * (each complex element = 2 multifloats::float64x2s, interleaved re,im).
 *
 * Ap layout (TCOPY of normal A or NCOPY of trans A, both shapes):
 *   per kernel i-slice (one MR=2 row strip of C), the data is bk
 *   K-rows × 4 multifloats::float64x2s, with each K-row = [r0_re, r0_im, r1_re,
...
void mblas_ygemm_kernel(ptrdiff_t bm, ptrdiff_t bn, ptrdiff_t bk,
                        multifloats::float64x2 alphar, multifloats::float64x2 alphai,
                        const multifloats::float64x2 *Ap,
                        const multifloats::float64x2 *Bp,
                        multifloats::float64x2 *C, ptrdiff_t ldc);
...
void mblas_ygemm_ncopy(ptrdiff_t m, ptrdiff_t n,
                       int conj,
                       const multifloats::float64x2 *a, ptrdiff_t lda,
 ...
 *
 * Same `conj` semantics and lda convention as mblas_ygemm_ncopy.
 */
...
void mblas_ygemm_beta(ptrdiff_t m, ptrdiff_t n,
                      multifloats::float64x2 beta_r, multifloats::float64x2 beta_i,
                      multifloats::float64x2 *c, ptrdiff_t ldc);
 ...
 * see mblas_l3_real.h. For SYMM (NOT hemm) there is no conjugation,
 ...
 */
void mblas_ysyrk_beta_u(ptrdiff_t n,
                        multifloats::float64x2 beta_r, multifloats::float64x2 beta_i,
                        multifloats::float64x2 *c, ptrdiff_t ldc);
void mblas_ysyrk_beta_l(ptrdiff_t n,
                        multifloats::float64x2 beta_r, multifloats::float64x2 beta_i,
                        multifloats::float64x2 *c, ptrdiff_t ldc);
 ...
 */
void mblas_yherk_beta_u(ptrdiff_t n, multifloats::float64x2 beta_r,
                        multifloats::float64x2 *c, ptrdiff_t ldc);
void mblas_yherk_beta_l(ptrdiff_t n, multifloats::float64x2 beta_r,
                        multifloats::float64x2 *c, ptrdiff_t ldc);

/* ── HERK kernel: SYRK kernel with diagonal-imag clamp ───────────────
 *
 * Port of OpenBLAS driver/level3/zherk_kernel.c. Identical to
 * mblas_ysyrk_kernel_{u,l} *except* that the diagonal element of each
 ...
 * mblas_ysyr2k_kernel_{u,l}; differs only in the diagonal NR×NR sub-
 * block writeback:
 *
...
void mblas_ytrmm_iutcopy(ptrdiff_t m, ptrdiff_t n,
                         const multifloats::float64x2 *a, ptrdiff_t lda,
                         ptrdiff_t posX, ptrdiff_t posY,
                         multifloats::float64x2 *b, int unit, int conj);
 ...
 * mblas_ygemm_kernel). LEFT and TRANSA macros converted to runtime
 * `left` and `trans` flags. C := alpha * ba * bb (overwrite), per-
 * element 2 multifloats::float64x2s (interleaved re,im).
 */
void mblas_ytrmm_kernel(int left, int trans,
                        ptrdiff_t bm, ptrdiff_t bn, ptrdiff_t bk,
                        multifloats::float64x2 alphar, multifloats::float64x2 alphai,
                        const multifloats::float64x2 *ba,
                        const multifloats::float64x2 *bb,
                        multifloats::float64x2 *C, ptrdiff_t ldc,
                        ptrdiff_t offset);
 ...
 * complex tile, then call mblas_ygemm_kernel.
 */
...
void mblas_ytrsm_iutcopy(ptrdiff_t m, ptrdiff_t n,
                         const multifloats::float64x2 *a, ptrdiff_t lda,
                         ptrdiff_t offset, multifloats::float64x2 *b,
                         int unit, int conj);
 ...
 *
 * Per-element width = 2 multifloats::float64x2s (interleaved re,im); ldc, k, m,
