/*
 * qsyr2k — kind16 (REAL(KIND=16) / 128-bit __float128) port of OpenBLAS DSYR2K.
 *
 ...
 *                                       walk per (is, js) tile)
 *   - driver/level3/syr2k_kernel.c     (qblas_esyr2k_kernel_{u,l} in
 ...
 *
 * Triangular beta reuses the SYRK helpers (qblas_esyrk_beta_{u,l}) —
    ...
    T *c, const int *ldc_)
{
    ...
    if (uplo == 'U') qblas_esyrk_beta_u((ptrdiff_t)N, beta, c, (ptrdiff_t)ldc);
    else             qblas_esyrk_beta_l((ptrdiff_t)N, beta, c, (ptrdiff_t)ldc);
    ...
    /* SYR2K does ~ N^2 * K flops; tiny-cutoff sized like qgemm. */
        ...
        for (int js = 0; js < N; js += NC) {
            ...
            for (int ls = 0; ls < K; ls += KC) {
                ...
                for (int is = m_lo_eff; is < m_hi_eff; is += MC) {
                    ...
                    if (uplo == 'U') {
                        qblas_esyr2k_kernel_u(min_i, jb, pb, alpha,
                            Ap_A, Bp_B,
                            &c[(size_t)js * ldc + is], ldc,
                            (ptrdiff_t)(is - js), 1);
                    } else {
                        qblas_esyr2k_kernel_l(min_i, jb, pb, alpha,
                            Ap_A, Bp_B,
                            &c[(size_t)js * ldc + is], ldc,
                            (ptrdiff_t)(is - js), 1);
                    }
