/*
 * xtpsv — kind16 port of OpenBLAS ztpsv.  Complex packed triangular solve.
 *
 *   x := inv(op(A)) * x  (op = A, A^T, A^H).  Packed storage matches qtpsv.
 *
 * Fortran ABI:  subroutine xtpsv(uplo, trans, diag, n, ap, x, incx)
 */
