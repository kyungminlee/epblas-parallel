/*
 * xhbmv — kind16 port of OpenBLAS zhbmv (interface/zhbmv.c +
 ...
 *
 * Band storage matches qsbmv (identical to OpenBLAS).
 *
 * Fortran ABI:  subroutine xhbmv(uplo, n, k, alpha, a, lda, x, incx, beta, y, incy)
 */
                          ...
                          const C *x, C *yp)
{
    ...
    for (ptrdiff_t i = n_from; i < n_to; ++i) {
        ...
        for (ptrdiff_t j = 0; j < length; ++j)
            s += conjq(aoff[j]) * xoff[j];
        /* Diagonal: REAL(a[k, i]) * x[i] */
        R ajj_re = (R)crealq(acol[k]);
        yp[i] += s + ajj_re * xi;
        acol += lda;
    }
}
                          ...
                          const C *x, C *yp)
{
    ...
    for (ptrdiff_t i = n_from; i < n_to; ++i) {
        ...
        const C *xoff = &x[i + 1];
        for (ptrdiff_t j = 0; j < length; ++j)
            s += conjq(acol[1 + j]) * xoff[j];
        /* Diagonal: REAL(a[0, i]) * x[i] */
        R ajj_re = (R)crealq(acol[0]);
        yp[i] += s + ajj_re * xi;
        acol += lda;
    }
}
...
void xhbmv_(const char *UPLO, const int *N, const int *K, const C *ALPHA,
            ...
            const C *BETA, C *y, const int *INCY)
{
    ...
    if (n == 0 || (alpha == 0.0Q && beta == 1.0Q)) return;
    int upper = (toupper((unsigned char)*UPLO) == 'U');

    if (beta != 1.0Q) {
        ptrdiff_t absy = incy < 0 ? -incy : incy;
        ptrdiff_t iy = 0;
        if (beta == 0.0Q) for (ptrdiff_t i = 0; i < n; ++i) { y[iy] = 0.0Q; iy += absy; }
    ...
    if (incx == 1 && incy == 1) {
        if (upper) {
            for (ptrdiff_t j = 0; j < n; ++j) {
                ...
                R ajj = (R)crealq(a[k + (size_t)j * lda]);
                y[j] += t1 * ajj + alpha * t2;
            }
        } else {
            for (ptrdiff_t j = 0; j < n; ++j) {
