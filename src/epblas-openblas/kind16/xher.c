/*
 * xher — kind16 port of OpenBLAS zher.  Hermitian rank-1 update.
 *
 ...
 *
 * Fortran ABI:  subroutine xher(uplo, n, alpha, x, incx, a, lda)
 */
...
void xher_(const char *UPLO, const int *N, const R *ALPHA,
    ...
    if (incx == 1) {
                ...
                #pragma omp parallel num_threads(num_cpu)
                {
                    ...
                    if (upper) {
                        for (ptrdiff_t j = m_from; j < m_to; ++j) {
                            C *aj = &A_(0, j);
                            if (x[j] != 0.0Q) {
                                C temp = (C)alpha * conjq(x[j]);
                                for (ptrdiff_t i = 0; i < j; ++i) aj[i] += x[i] * temp;
                                aj[j] = (R)crealq(aj[j]) + (R)crealq(x[j] * temp);
                            } else {
                                aj[j] = (R)crealq(aj[j]);
                            }
                        }
                        ...
                        for (ptrdiff_t j = m_from; j < m_to; ++j) {
                            ...
                            if (x[j] != 0.0Q) {
                                C temp = (C)alpha * conjq(x[j]);
                                aj[j] = (R)crealq(aj[j]) + (R)crealq(temp * x[j]);
        ...
        if (upper) {
            ...
            for (ptrdiff_t j = 0; j < n; ++j) {
                C *aj = &A_(0, j);
                if (x[jx] != 0.0Q) {
                    C temp = (C)alpha * conjq(x[jx]);
                    ptrdiff_t ix = 0;
                    for (ptrdiff_t i = 0; i < j; ++i) { aj[i] += x[ix] * temp; ix += incx; }
                    aj[j] = (R)crealq(aj[j]) + (R)crealq(x[jx] * temp);
                } else {
                    aj[j] = (R)crealq(aj[j]);
                }
                jx += incx;
            ...
            for (ptrdiff_t j = 0; j < n; ++j) {
                ...
                if (x[jx] != 0.0Q) {
                    C temp = (C)alpha * conjq(x[jx]);
                    aj[j] = (R)crealq(aj[j]) + (R)crealq(temp * x[jx]);
                    ptrdiff_t ix = jx;
