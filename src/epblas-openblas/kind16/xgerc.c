/*
 * xgerc — kind16 port of OpenBLAS zgerc.  Complex conjugated outer product.
 *
 *   A := alpha * x * y^H + A    (conjugate y)
 *
 * Fortran ABI:  subroutine xgerc(m, n, alpha, x, incx, y, incy, a, lda)
 */
...
void xgerc_(const int *M, const int *N, const C *ALPHA,
