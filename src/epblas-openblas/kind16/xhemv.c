/*
 * xhemv — kind16 port of OpenBLAS zhemv.  Hermitian MV.
 *
 ...
 *
 * Fortran ABI:  subroutine xhemv(uplo, n, alpha, a, lda, x, incx, beta, y, incy)
 */
...
void xhemv_(const char *UPLO, const int *N, const C *ALPHA,
            ...
            const C *BETA, C *y, const int *INCY)
{
    ...
    if (upper) {
        for (ptrdiff_t j = 0; j < n; ++j) {
            ...
            for (ptrdiff_t i = 0; i < j; ++i) {
                y[iy] += t1 * A_(i, j);
                t2    += conjq(A_(i, j)) * x[ix];
                ix += incx; iy += incy;
            }
            y[jy] += t1 * (R)crealq(A_(j, j)) + alpha * t2;
            jx += incx; jy += incy;
        }
    } else {
        for (ptrdiff_t j = 0; j < n; ++j) {
