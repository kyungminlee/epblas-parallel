/*
 * qtrsm — kind16 (REAL(KIND=16) / 128-bit __float128) port of OpenBLAS DTRSM.
 *
 *   op(A) * X = alpha * B    (SIDE='L')   →  B := alpha * inv(op(A)) * B_old
 *   X * op(A) = alpha * B    (SIDE='R')   →  B := alpha * B_old * inv(op(A))
 *
 * Faithful L3-blocked port (sibling of qtrmm.c). Full OpenBLAS NC×KC×MC
 ...
 *                                      → qblas_etrsm_kernel (collapsed)
 *   - kernel/generic/trsm_{ut,un,lt,ln}copy_2.c
 *                                      → qblas_etrsm_i{ut,un,lt,ln}copy
 *
 * Algorithm: TRSM is implemented as
 *   1. alpha pre-scale of B in-place (qblas_egemm_beta with beta=alpha).
                                             ...
                                             ptrdiff_t offset, T *bp)
{
    ...
    if (!trans) {
        qblas_etrsm_iltcopy(m, n, a, lda, offset, bp, unit);
    } else {
        qblas_etrsm_iuncopy(m, n, a, lda, offset, bp, unit);
    }
}
                                              ...
                                              ptrdiff_t offset, T *bp)
{
    ...
    if (!trans) {
        qblas_etrsm_iutcopy(m, n, a, lda, offset, bp, unit);
    } else {
        qblas_etrsm_ilncopy(m, n, a, lda, offset, bp, unit);
    }
}
        ...
        if (forward) {
            ...
            for (int ls = 0; ls < m; ls += KC) {
                ...
                for (int is = ls + min_l; is < m; is += MC) {
                    ...
                    qblas_egemm_kernel(min_i, min_j, min_l, dm1,
                                       Ap, Bp,
                                       &b[(size_t)is + (size_t)js * ldb], ldb);
                }
            }
        ...
        for (int js = 0; js < N; js += NC) {
            ...
            for (int ls = 0; ls < js; ls += KC) {
                ...
                for (int jjs = js; jjs < js + min_j; jjs += NR) {
                    ...
                    qblas_egemm_kernel(min_i, min_jj, min_l, dm1,
            ...
            for (int ls = js; ls < js + min_j; ls += KC) {
                ...
                for (int is = min_i; is < m_band; is += MC) {
                    ...
                    if (min_j - min_l + js - ls > 0) {
                        qblas_egemm_kernel(min_i, min_j - min_l + js - ls, min_l, dm1,
            ...
            for (int ls = start_ls; ls >= js - min_j; ls -= KC) {
                ...
                for (int is = min_i; is < m_band; is += MC) {
                    ...
                    qblas_etrsm_kernel(/*left=*/0, kt,
                                       ...
                                       /*offset=*/0);
                    if (min_j - js + ls > 0) {
                        qblas_egemm_kernel(min_i, min_j - js + ls, min_l, dm1,
                                           sa, sb,
                                           &b[(size_t)(m_lo + is) + (size_t)(js - min_j) * ldb], ldb);
                    }
                }
    ...
    T *b, const int *ldb_)
{
    ...
    /* Adaptive MC for small K (mirrors qgemm/qtrmm). For SIDE=L K = M;
     * for SIDE=R K = N. */
    int K_eff = lside ? M : N;
    int MC = MC0;
    if (K_eff <= KC) {
