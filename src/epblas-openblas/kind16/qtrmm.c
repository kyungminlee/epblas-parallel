/*
 * qtrmm — kind16 (REAL(KIND=16) / 128-bit __float128) port of OpenBLAS DTRMM.
 *
 ...
 *   - kernel/generic/trmmkernel_2x2.c → qblas_etrmm_kernel (shared)
 *   - kernel/generic/trmm_{ut,un,lt,ln}copy_2.c
 *                                     → qblas_etrmm_i{ut,un,lt,ln}copy
 *
 * Algorithm reconstruction (from interface/trsm.c lines 64+, trmm_L.c
 * lines 92, 103-113): TRMM is implemented as
 *   1. alpha pre-scale B in-place (qblas_egemm_beta with beta=alpha)
 *   2. L3 nest runs with kernel-alpha = 1.0Q, overwriting B tile by tile.
 ...
 * overwrite). Our shared qblas_egemm_kernel does +=, so we use
 * qblas_egemm_kernel_store (which zeros then calls the shared kernel).
 *
 * Fortran ABI:
...
{
    if (side_l) {
        if (uplo_upper && !trans)       qblas_etrmm_iutcopy(m, n, a, lda, posX, posY, bp, unit);
        else if (uplo_upper &&  trans)  qblas_etrmm_iuncopy(m, n, a, lda, posX, posY, bp, unit);
        else if (!uplo_upper && !trans) qblas_etrmm_iltcopy(m, n, a, lda, posX, posY, bp, unit);
        else                            qblas_etrmm_ilncopy(m, n, a, lda, posX, posY, bp, unit);
    } else {
        if (uplo_upper && !trans)       qblas_etrmm_iuncopy(m, n, a, lda, posX, posY, bp, unit);
        else if (uplo_upper &&  trans)  qblas_etrmm_iutcopy(m, n, a, lda, posX, posY, bp, unit);
        else if (!uplo_upper && !trans) qblas_etrmm_ilncopy(m, n, a, lda, posX, posY, bp, unit);
        else                            qblas_etrmm_iltcopy(m, n, a, lda, posX, posY, bp, unit);
    }
}
    ...
    for (int js = js0; js < js1; js += NC) {
        ...
        if ((upper && !trans) || (!upper && trans)) {
            ...
            for (int ls = min_l; ls < m; ls += KC) {
                 ...
                 * for TRANS use NCOPY. */
                if (!trans) {
                    qblas_egemm_tcopy(min_l, min_i,
                                      &a[(size_t)ls * lda], lda, Ap);
                } else {
                    qblas_egemm_ncopy(min_l, min_i,
                                      &a[(size_t)ls], lda, Ap);
                }
                ...
                for (int jjs = js; jjs < js + min_j; jjs += NR) {
                    ...
                    qblas_egemm_kernel_store(min_i, min_jj, min_l, dp1,
                                             Ap, Bp + (size_t)min_l * (jjs - js),
                                             &b[(size_t)jjs * ldb], ldb);
                }
                ...
                for (int is = min_i; is < ls; is += min_i) {
                    ...
                    qblas_egemm_kernel_store(min_i, min_j, min_l, dp1,
                                             Ap, Bp,
                                             &b[(size_t)is + (size_t)js * ldb], ldb);
                }
            ...
            for (int ls = js; ls < js + min_j; ls += KC) {
                ...
                for (int jjs = 0; jjs < ls - js; jjs += NR) {
                    ...
                    if (!trans) {
                        ...
                        qblas_egemm_tcopy(min_l, min_jj,
                ...
                for (int jjs = 0; jjs < min_l; jjs += NR) {
                    ...
                    qblas_etrmm_kernel(/*left=*/0, /*trans=*/kt,
                                       min_i, min_jj, min_l, dp1,
                ...
                for (int is = min_i; is < m_band; is += MC) {
                    ...
                    qblas_egemm_kernel_store(min_i, ls - js, min_l, dp1,
                                             sa, sb,
        ...
        for (int js = N; js > 0; js -= NC) {
            ...
            for (int ls = start_ls; ls >= js - min_j; ls -= KC) {
                ...
                for (int is = min_i; is < m_band; is += MC) {
                    ...
                    if (js - ls - min_l > 0) {
                        qblas_egemm_kernel_store(min_i, js - ls - min_l, min_l, dp1,
    ...
    size_t side_len, size_t uplo_len, size_t transa_len, size_t diag_len)
{
    ...
    /* Adaptive MC for small K (mirrors qgemm). For SIDE='L' K = M; for
     * SIDE='R' K = N. */
    int K_eff = lside ? M : N;
    int MC = MC0;
    if (K_eff <= KC) {
     ...
     * panels. Sizes mirror qgemm but multiplied by 2 to comfortably hold
