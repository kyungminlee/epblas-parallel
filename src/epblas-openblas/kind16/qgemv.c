/*
 * qgemv — kind16 (REAL(KIND=16) / 128-bit __float128) port of OpenBLAS dgemv.
 *
 ...
 *     (x86_64 has no AVX path for 128-bit __float128).
 *
            ...
            const T *BETA, T *y, const int *INCY)
{
    ...
    if (beta != 1.0Q) {
        ...
        ptrdiff_t iy = 0;
        if (beta == 0.0Q) {
            for (ptrdiff_t i = 0; i < leny; ++i) { y[iy] = 0.0Q; iy += absy; }
        } else {
