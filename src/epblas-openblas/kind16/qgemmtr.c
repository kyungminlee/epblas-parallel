/*
 * qgemmtr — kind16 (REAL(KIND=16) / 128-bit __float128) port of OpenBLAS DGEMMT.
 *
 ...
 * the single GEMV, columns serialized). We call qgemv_ which already
 * has internal OMP; the column loop is sequential to avoid nested
 * parallelism. qgemv_ has its own work-threshold for OMP=1 fallback
 * when (j*k) is tiny.
 *
 * Fortran ABI:
 *   subroutine qgemmtr(uplo, transa, transb, n, k, alpha, a, lda,
 *                      b, ldb, beta, c, ldc)
 */
