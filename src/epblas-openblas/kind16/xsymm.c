/*
 * xsymm — kind16 (COMPLEX(KIND=16)) port of OpenBLAS ZSYMM.
 *
 ...
 * HEMM is a separate routine (xhemm — task #64).
 *
 ...
 *                                 with COMPSIZE=2)
 *   - kernel/generic/
 *       zsymm_ucopy_2.c          → qblas_ysymm_ucopy  (shared via common/)
 *       zsymm_lcopy_2.c          → qblas_ysymm_lcopy  (shared via common/)
 *
 * The microkernel + standard GEMM packers (qblas_ygemm_kernel /
 * qblas_ygemm_ncopy / qblas_ygemm_tcopy / qblas_ygemm_beta) are reused
 * from the shared common/qblas_l3_complex.c. SYMM packers do NOT
 * negate the imag float (no conjugation).
 *
 * Fortran ABI:
 *   subroutine xsymm(side, uplo, m, n, alpha, a, lda, b, ldb,
 *                    beta, c, ldc)
 *   - character args with trailing hidden size_t lengths (gfortran)
 *   - alpha, beta are COMPLEX(KIND=16): 2 __float128s each (re, im)
 *   - a, b, c are COMPLEX(KIND=16) arrays (interleaved re,im)
 *   - lda, ldb, ldc are in COMPLEX(KIND=16) elements
 */
    ...
    T *c, const int *ldc_)
{
    ...
    /* Beta pre-pass. */
    qblas_ygemm_beta((ptrdiff_t)M, (ptrdiff_t)N, beta_r, beta_i,
                     c, (ptrdiff_t)(*ldc_));
    ...
    /* Complex elements → 2 __float128s per element. */
            ...
            for (int ls = 0; ls < K; ls += KC) {
...
#endif
                {
                    if (side == 'L') {
                        ...
                        if (uplo == 'U') {
                            qblas_ysymm_ucopy(pb, jb,
                                B_eff, ldb_eff, js, ls, Bp);
                        } else {
                            qblas_ysymm_lcopy(pb, jb,
                                B_eff, ldb_eff, js, ls, Bp);
                        }
                    }
                }
                ...
                for (int is = m_lo; is < m_hi; is += MC) {
                    ...
                    if (side == 'L') {
                        if (uplo == 'U') {
                            qblas_ysymm_ucopy(pb, min_i,
                                A_eff, lda_eff, is, ls, Ap);
                        } else {
                            qblas_ysymm_lcopy(pb, min_i,
                                A_eff, lda_eff, is, ls, Ap);
                        }
                    } else {
