/*
 * xher2k — kind16 (COMPLEX(KIND=16)) Hermitian rank-2k update.
 *
 *   C := alpha*A*B^H + conj(alpha)*B*A^H + beta*C   (trans='N', A,B are N×K)
 *   C := alpha*A^H*B + conj(alpha)*B^H*A + beta*C   (trans='C', A,B are K×N)
 *
 * alpha is COMPLEX(KIND=16); beta is REAL(KIND=16). C is Hermitian:
 ...
 *                                     conj(alpha)·B·A^H)
 *   - driver/level3/zher2k_kernel.c → qblas_yher2k_kernel_{u,l}
 ...
 *                                     so we reuse qblas_yherk_beta_{u,l}
 *                                     directly.
 *
 ...
 *
 * The diagonal mirror lives inside qblas_yher2k_kernel — it computes
 ...
 *   - beta  is REAL(KIND=16): 1 __float128
        ...
        for (int js = 0; js < N; js += NC) {
            ...
            for (int ls = 0; ls < K; ls += KC) {
...
#endif
                {
                    ...
                    if (trans == 'N') {
                        qblas_ygemm_tcopy(pb, jb, conj_b_pack,
                            &a[((size_t)ls * lda + js) * 2], lda, Bp_A);
                        qblas_ygemm_tcopy(pb, jb, conj_b_pack,
                            &b[((size_t)ls * ldb + js) * 2], ldb, Bp_B);
                    } else {
                        qblas_ygemm_ncopy(pb, jb, conj_b_pack,
                ...
                for (int is = m_lo_eff; is < m_hi_eff; is += MC) {
                    ...
                    if (trans == 'N') {
                        qblas_ygemm_tcopy(pb, min_i, conj_a_pack,
                            &a[((size_t)ls * lda + is) * 2], lda, Ap_A);
                        qblas_ygemm_tcopy(pb, min_i, conj_a_pack,
                            &b[((size_t)ls * ldb + is) * 2], ldb, Ap_B);
                    } else {
                        qblas_ygemm_ncopy(pb, min_i, conj_a_pack,
                    ...
                    if (uplo == 'U') {
                        qblas_yher2k_kernel_u(min_i, jb, pb, alphar, alphai,
                            Ap_A, Bp_B,
                        ...
                        qblas_yher2k_kernel_l(min_i, jb, pb, alphar, alphai,
                            Ap_A, Bp_B,
                            &c[((size_t)js * ldc + is) * 2], ldc,
                            (ptrdiff_t)(is - js), 1);
                        qblas_yher2k_kernel_l(min_i, jb, pb, alphar, -alphai,
