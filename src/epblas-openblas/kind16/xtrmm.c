/*
 * xtrmm — kind16 (COMPLEX(KIND=16)) port of OpenBLAS ZTRMM.
 *
 *   B := alpha * op(A) * B    (SIDE='L')
 *   B := alpha * B * op(A)    (SIDE='R')
 *
 * Faithful L3-blocked port (complex twin of qtrmm.c).
 *
 ...
 *   - kernel/generic/ztrmmkernel_2x2.c → qblas_ytrmm_kernel (NN-only;
 *                                        conjugation absorbed into the
 *                                        packers via the `conj` flag)
 *   - kernel/generic/ztrmm_{ut,un,lt,ln}copy_2.c
 *                                     → qblas_ytrmm_i{ut,un,lt,ln}copy
 *
 * Same algorithmic structure as qtrmm.c — see that file's banner for
 * the (uplo, trans, side) → packer/kernel routing.
 *
 * Complex notes:
 *   - Per-element width = 2 __float128s (interleaved re,im).
 *   - lda, ldb in COMPLEX(KIND=16) elements; the kernels and packers
 ...
 *     the kernel always runs the NN form (matches xgemm pattern).
 *
 * Fortran ABI:
 ...
 *   - a, b are COMPLEX(KIND=16) arrays (interleaved re,im)
 *   - lda, ldb are in COMPLEX(KIND=16) elements
 */
...
/* Same dispatch logic as qtrmm.c's pack_trmm_a — see comment there for
...
{
    if (side_l) {
        if (uplo_upper && !trans)       qblas_ytrmm_iutcopy(m, n, a, lda, posX, posY, bp, unit, conj);
        else if (uplo_upper &&  trans)  qblas_ytrmm_iuncopy(m, n, a, lda, posX, posY, bp, unit, conj);
        else if (!uplo_upper && !trans) qblas_ytrmm_iltcopy(m, n, a, lda, posX, posY, bp, unit, conj);
        else                            qblas_ytrmm_ilncopy(m, n, a, lda, posX, posY, bp, unit, conj);
    } else {
        if (uplo_upper && !trans)       qblas_ytrmm_iuncopy(m, n, a, lda, posX, posY, bp, unit, conj);
        else if (uplo_upper &&  trans)  qblas_ytrmm_iutcopy(m, n, a, lda, posX, posY, bp, unit, conj);
        else if (!uplo_upper && !trans) qblas_ytrmm_ilncopy(m, n, a, lda, posX, posY, bp, unit, conj);
        else                            qblas_ytrmm_iltcopy(m, n, a, lda, posX, posY, bp, unit, conj);
    }
}
        ...
        if ((upper && !trans) || (!upper && trans)) {
            ...
            for (int ls = min_l; ls < m; ls += KC) {
                ...
                if (!trans) {
                    ...
                    qblas_ygemm_ncopy(min_l, min_i, /*conj=*/conj,
                ...
                for (int jjs = js; jjs < js + min_j; jjs += NR) {
                    ...
                    qblas_ygemm_kernel_store(min_i, min_jj, min_l, dp1r, dp1i,
                                             Ap, Bp + (size_t)min_l * (jjs - js) * 2,
                                             &b[(size_t)jjs * ldb * 2], ldb);
                }
                ...
                for (int is = min_i; is < ls; is += min_i) {
                    ...
                    qblas_ygemm_kernel_store(min_i, min_j, min_l, dp1r, dp1i,
                                             Ap, Bp,
            ...
            for (int ls = js; ls < js + min_j; ls += KC) {
                ...
                for (int jjs = 0; jjs < ls - js; jjs += NR) {
                    ...
                    if (!trans) {
                        ...
                        qblas_ygemm_tcopy(min_l, min_jj, /*conj=*/conj,
                ...
                for (int is = min_i; is < m_band; is += MC) {
                    ...
                    qblas_ygemm_kernel_store(min_i, ls - js, min_l, dp1r, dp1i,
                                             sa, sb,
        ...
        for (int js = N; js > 0; js -= NC) {
            ...
            for (int ls = start_ls; ls >= js - min_j; ls -= KC) {
                ...
                for (int is = min_i; is < m_band; is += MC) {
                    ...
                    if (js - ls - min_l > 0) {
                        qblas_ygemm_kernel_store(min_i, js - ls - min_l, min_l, dp1r, dp1i,
    ...
    T *b, const int *ldb_)
{
    ...
    if (alphar == 0.0Q && alphai == 0.0Q) {
        qblas_ygemm_beta((ptrdiff_t)M, (ptrdiff_t)N, alphar, alphai, b, (ptrdiff_t)ldb);
        return;
    }
    ...
    /* Buffer sizes — complex = 2 __float128s per element. */
    ...
    if (!Ap_arr || !Bp_arr) {
        ...
        fprintf(stderr, "xtrmm: pack buffer allocation failed\n");
        abort();
    }
    int alloc_ok = 1;
