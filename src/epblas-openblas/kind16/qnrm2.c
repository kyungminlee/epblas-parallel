/*
 * qnrm2 — kind16 port of LAPACK 3.12.1 dnrm2 (Blue's algorithm).
 *
 * Three-accumulator scaled sum-of-squares to avoid overflow/underflow.
 * Reference: blas/src/qnrm2.f90.
 *
 * Scaling constants computed once per process from <float.h> /
 * ldexpq() so they survive across calls without recomputing.
 */
#include <stddef.h>
#include <quadmath.h>
#include <math.h>
...
static void blue_init(void)
{
    ...
    /* Use safer compute via ldexpq directly with the exact LAPACK
     * formula:
...
T qnrm2_(const int *N, const T *x, const int *INCX)
{
