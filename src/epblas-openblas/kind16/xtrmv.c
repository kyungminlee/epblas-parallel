/*
 * xtrmv — kind16 port of OpenBLAS ztrmv (interface/trmv.c +
 ...
 *
 * Fortran ABI:  subroutine xtrmv(uplo, trans, diag, n, a, lda, x, incx)
 */
    ...
    for (ptrdiff_t is = m_from; is < m_to; is += TB) {
        ...
        if (upper && is > 0) {
            for (ptrdiff_t j = is; j < is + min_i; ++j) {
                C s = 0.0Q;
                const C *col = &A_(0, j);
                for (ptrdiff_t i = 0; i < is; ++i) s += conjq(col[i]) * x[i];
                y[j] += s;
            }
        }
        for (ptrdiff_t i = is; i < is + min_i; ++i) {
            if (upper && i > is) {
                C s = 0.0Q;
                const C *col = &A_(0, i);
                for (ptrdiff_t k = is; k < i; ++k) s += conjq(col[k]) * x[k];
                y[i] += s;
            }
            if (nounit) y[i] += conjq(A_(i, i)) * x[i];
            else        y[i] += x[i];
            if (!upper && i + 1 < is + min_i) {
                C s = 0.0Q;
                const C *col = &A_(0, i);
                for (ptrdiff_t k = i + 1; k < is + min_i; ++k) s += conjq(col[k]) * x[k];
                y[i] += s;
            }
        }
        if (!upper && is + min_i < n) {
            for (ptrdiff_t j = is; j < is + min_i; ++j) {
                C s = 0.0Q;
                const C *col = &A_(0, j);
                for (ptrdiff_t i = is + min_i; i < n; ++i) s += conjq(col[i]) * x[i];
                y[j] += s;
            }
        }
    }
...
void xtrmv_(const char *UPLO, const char *TRANS, const char *DIAG,
            ...
            if (incx == 1) {
                for (ptrdiff_t j = n - 1; j >= 0; --j) {
                    ...
                    if (noconj) {
                        ...
                        if (nounit) temp *= conjq(aj[j]);
                        for (ptrdiff_t i = j - 1; i >= 0; --i) temp += conjq(aj[i]) * x[i];
                    }
                    x[j] = temp;
                }
            } else {
                ...
                for (ptrdiff_t j = n - 1; j >= 0; --j) {
                    ...
                    if (noconj) {
                        ...
                        if (nounit) temp *= conjq(aj[j]);
                        for (ptrdiff_t i = j - 1; i >= 0; --i) { ix -= incx; temp += conjq(aj[i]) * x[ix]; }
                    }
                    x[jx] = temp;
                    jx -= incx;
                }
            ...
            if (incx == 1) {
                ...
                for (ptrdiff_t j = 0; j < n; ++j) {
                    ...
                    if (noconj) {
                        ...
                        if (nounit) temp *= conjq(aj[j]);
                        for (ptrdiff_t i = j + 1; i < n; ++i) { ix += incx; temp += conjq(aj[i]) * x[ix]; }
                    }
                    x[jx] = temp;
                    jx += incx;
                }
