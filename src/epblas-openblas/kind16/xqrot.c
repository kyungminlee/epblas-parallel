/*
 * xqrot — kind16 port of OpenBLAS zdrot.
 *
 * Apply real-coefficient plane rotation to complex vectors:
 *   x' :=  c*x + s*y
 *   y' := -s*x + c*y
 * where c, s are REAL(KIND=16) and x, y are COMPLEX(KIND=16).
 *
 * Fortran ABI: xqrot(N, ZX, INCX, ZY, INCY, C, S).
 */
#include <stddef.h>
#include <quadmath.h>
#ifdef _OPENMP
...
void xqrot_(const int *N, C *x, const int *INCX, C *y, const int *INCY,
