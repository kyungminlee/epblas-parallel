/*
 * xsyrk — kind16 (COMPLEX(KIND=16)) port of OpenBLAS ZSYRK.
 *
    ...
    T *c, const int *ldc_)
{
    ...
    if (uplo == 'U') qblas_ysyrk_beta_u((ptrdiff_t)N, beta_r, beta_i,
                                        c, (ptrdiff_t)ldc);
    else             qblas_ysyrk_beta_l((ptrdiff_t)N, beta_r, beta_i,
                                        c, (ptrdiff_t)ldc);
    ...
    /* Complex packing: 2 __float128s per element. */
            ...
            for (int ls = 0; ls < K; ls += KC) {
...
#endif
                {
                    if (trans == 'N') {
                         ...
                         * trans-pack shape (same as qgemm tb='T'). */
                        qblas_ygemm_tcopy(pb, jb, 0,
                ...
                for (int is = m_lo_eff; is < m_hi_eff; is += MC) {
                    ...
                    if (trans == 'N') {
                        ...
                        qblas_ygemm_ncopy(pb, min_i, 0,
                            &a[((size_t)is * lda + ls) * 2], lda, Ap);
                    }

                    if (uplo == 'U') {
                        qblas_ysyrk_kernel_u(min_i, jb, pb, alphar, alphai,
                            Ap, Bp,
                            &c[((size_t)js * ldc + is) * 2], ldc,
                            (ptrdiff_t)(is - js));
                    } else {
                        qblas_ysyrk_kernel_l(min_i, jb, pb, alphar, alphai,
                            Ap, Bp,
                            &c[((size_t)js * ldc + is) * 2], ldc,
                            (ptrdiff_t)(is - js));
                    }
