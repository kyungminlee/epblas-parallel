/*
 * qtbsv — kind16 port of OpenBLAS dtbsv.  Banded triangular solve.
 *
 *   x := inv(op(A)) * x   where A is N x N triangular banded with K extra diagonals
 *
 * Band storage matches qtbmv. Loop directions match Fortran reference.
 *
 * Fortran ABI:  subroutine qtbsv(uplo, trans, diag, n, k, a, lda, x, incx)
 */
