/*
 * xherk — kind16 (COMPLEX(KIND=16)) Hermitian rank-k update.
 *
 *   C := alpha * A * A^H + beta * C    (trans='N', A is N×K)
 *   C := alpha * A^H * A + beta * C    (trans='C', A is K×N)
 *
 * alpha and beta are REAL(KIND=16) scalars. C is Hermitian: only the
 ...
 *                                → qblas_yherk_kernel_{u,l}
 *                                  (in common/qblas_l3_complex.c)
 *   - driver/level3/zherk_beta.c → qblas_yherk_beta_{u,l}
 *                                  (in common/qblas_l3_complex.c)
 *
 ...
 * Our shared qblas_ygemm_kernel is NN-only (matching xgemm/xsymm —
 * conjugation absorbed at pack time), so we set `conj` on the side the
 * upstream kernel would have conjugated:
 *   - TRANS='N': Bp packer uses conj=1 (caller of xherk path passes A
 ...
 *   - alpha, beta are REAL(KIND=16): one __float128 each
 *   - a, c are COMPLEX(KIND=16) arrays (interleaved re,im)
 *   - lda, ldc are in COMPLEX(KIND=16) elements
 */
    ...
    size_t uplo_len, size_t trans_len)
{
    ...
    if (uplo == 'U') qblas_yherk_beta_u((ptrdiff_t)N, beta_r,
                                        c, (ptrdiff_t)ldc);
    else             qblas_yherk_beta_l((ptrdiff_t)N, beta_r,
                                        c, (ptrdiff_t)ldc);
        ...
        for (int js = 0; js < N; js += NC) {
            ...
            for (int ls = 0; ls < K; ls += KC) {
...
#endif
                {
                    ...
                    if (trans == 'N') {
                        qblas_ygemm_tcopy(pb, jb, conj_b,
                            &a[((size_t)ls * lda + js) * 2], lda, Bp);
                    } else {
                        qblas_ygemm_ncopy(pb, jb, conj_b,
                ...
                for (int is = m_lo_eff; is < m_hi_eff; is += MC) {
                    ...
                    if (trans == 'N') {
                        qblas_ygemm_tcopy(pb, min_i, conj_a,
                            &a[((size_t)ls * lda + is) * 2], lda, Ap);
                    } else {
                        qblas_ygemm_ncopy(pb, min_i, conj_a,
                            &a[((size_t)is * lda + ls) * 2], lda, Ap);
                    }

                    if (uplo == 'U') {
                        qblas_yherk_kernel_u(min_i, jb, pb, alphar,
                            Ap, Bp,
                            &c[((size_t)js * ldc + is) * 2], ldc,
                            (ptrdiff_t)(is - js));
                    } else {
                        qblas_yherk_kernel_l(min_i, jb, pb, alphar,
                            Ap, Bp,
                            &c[((size_t)js * ldc + is) * 2], ldc,
                            (ptrdiff_t)(is - js));
                    }
