/*
 * qsyrk — kind16 (REAL(KIND=16) / 128-bit __float128) port of OpenBLAS DSYRK.
 *
 ...
 *   - driver/level3/syrk_kernel.c      (qblas_esyrk_kernel_{u,l} in
        ...
        for (int js = 0; js < N; js += NC) {
            ...
            for (int ls = 0; ls < K; ls += KC) {
                ...
                for (int is = m_lo_eff; is < m_hi_eff; is += MC) {
                    ...
                    /* ICOPY(A) — into Ap. Mirrors qgemm's ICOPY
                    ...
                    if (uplo == 'U') {
                        qblas_esyrk_kernel_u(min_i, jb, pb, alpha,
                            Ap, Bp,
                            &c[(size_t)js * ldc + is], ldc,
                            (ptrdiff_t)(is - js));
                    } else {
                        qblas_esyrk_kernel_l(min_i, jb, pb, alpha,
                            Ap, Bp,
                            &c[(size_t)js * ldc + is], ldc,
                            (ptrdiff_t)(is - js));
                    }
