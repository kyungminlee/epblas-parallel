/*
 * qgemm — kind16 (REAL(KIND=16) / 128-bit __float128) port of OpenBLAS DGEMM.
 *
 ...
 *   - kernel/generic/
 *       gemmkernel_2x2.c       → qblas_egemm_kernel  (shared via common/)
 *       gemm_ncopy_2.c          → qblas_egemm_ncopy   (shared via common/)
 *       gemm_tcopy_2.c          → qblas_egemm_tcopy   (shared via common/)
 *       gemm_beta.c             → qblas_egemm_beta    (shared via common/)
 *
 * Differences from upstream DGEMM:
 *   - No SIMD. x86_64 has no AVX path for 128-bit __float128; the
    ...
    T *c, const int *ldc_)
{
    ...
    qblas_egemm_beta((ptrdiff_t)M, (ptrdiff_t)N, beta, c, (ptrdiff_t)ldc);
    ...
    int MC = MC0;
    if (K <= KC) {
        const long L2_TARGET_BYTES = QBLAS_L2_TARGET_BYTES;  /* P-core L2 nominal */
