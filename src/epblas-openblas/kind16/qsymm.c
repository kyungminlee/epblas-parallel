/*
 * qsymm — kind16 (REAL(KIND=16) / 128-bit __float128) port of OpenBLAS DSYMM.
 *
 ...
 *   - kernel/generic/
 *       symm_ucopy_2.c           → qblas_esymm_ucopy  (shared via common/)
 *       symm_lcopy_2.c           → qblas_esymm_lcopy  (shared via common/)
 *
 * The microkernel + standard GEMM packers (qblas_egemm_kernel /
 * qblas_egemm_ncopy / qblas_egemm_tcopy / qblas_egemm_beta) are reused
 * from the shared common/qblas_l3_real.c — SYMM only changes WHICH
 * pack function fills Ap (SIDE=L) or Bp (SIDE=R).
 *
 * Differences from upstream DSYMM:
 *   - No SIMD (same reason as qgemm).
 *   - Single-level OpenMP over the M axis with per-thread Ap, shared Bp
 *     packed once per (jc, pc) under `omp single` (mirrors qgemm.c
 *     exactly — see that file for the barrier ordering).
 *
 * Fortran ABI:
 *   subroutine qsymm(side, uplo, m, n, alpha, a, lda, b, ldb,
 *                    beta, c, ldc)
    ...
    T *c, const int *ldc_)
{
    ...
    /* Beta pre-pass. */
    qblas_egemm_beta((ptrdiff_t)M, (ptrdiff_t)N, beta, c, (ptrdiff_t)(*ldc_));
...
#endif
    {
        ...
        for (int js = 0; js < N; js += NC) {
            ...
            for (int ls = 0; ls < K; ls += KC) {
                ...
                {
                    if (side == 'L') {
                        qblas_egemm_ncopy(pb, jb,
                            &B_eff[(size_t)js * ldb_eff + ls], ldb_eff, Bp);
                    } else {
                        if (uplo == 'U') {
                            qblas_esymm_ucopy(pb, jb,
                                B_eff, ldb_eff, js, ls, Bp);
                        } else {
                            qblas_esymm_lcopy(pb, jb,
                                B_eff, ldb_eff, js, ls, Bp);
                        }
                    }
                }
                ...
                for (int is = m_lo; is < m_hi; is += MC) {
                    ...
                    if (side == 'L') {
                        if (uplo == 'U') {
                            qblas_esymm_ucopy(pb, min_i,
                                A_eff, lda_eff, is, ls, Ap);
                        } else {
                            qblas_esymm_lcopy(pb, min_i,
                                A_eff, lda_eff, is, ls, Ap);
                        }
                    } else {
                        qblas_egemm_tcopy(pb, min_i,
                            &A_eff[(size_t)ls * lda_eff + is], lda_eff, Ap);
                    }

                    qblas_egemm_kernel(min_i, jb, pb, alpha,
                                       Ap, Bp,
