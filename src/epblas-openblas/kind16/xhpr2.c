/*
 * xhpr2 — kind16 port of OpenBLAS zhpr2.  Hermitian packed rank-2 update.
 *
 ...
 * forward-mapped.
 *
 * Fortran ABI:  subroutine xhpr2(uplo, n, alpha, x, incx, y, incy, ap)
 */
...
void xhpr2_(const char *UPLO, const int *N, const C *ALPHA,
            ...
            C *ap, size_t uplo_len)
{
                ...
                #pragma omp parallel num_threads(num_cpu)
                {
                    ...
                    if (upper) {
                        for (ptrdiff_t j = m_from; j < m_to; ++j) {
                            ...
                            if (x[j] != 0.0Q || y[j] != 0.0Q) {
                                C t1 = alpha * conjq(y[j]);
                                C t2 = conjq(alpha * x[j]);
                                for (ptrdiff_t i = 0; i < j; ++i)
                                    aj[i] += x[i] * t1 + y[i] * t2;
                                aj[j] = (R)crealq(aj[j]) + (R)crealq(x[j] * t1 + y[j] * t2);
                            } else {
                                aj[j] = (R)crealq(aj[j]);
                            }
                        }
    ...
    if (upper) {
        for (ptrdiff_t j = 0; j < n; ++j) {
            C *aj = &ap[(size_t)j * (size_t)(j + 1) / 2];
            if (x[jx] != 0.0Q || y[jy] != 0.0Q) {
                C t1 = alpha * conjq(y[jy]);
                C t2 = conjq(alpha * x[jx]);
                ptrdiff_t ix = kx, iy = ky;
                for (ptrdiff_t i = 0; i < j; ++i) {
                    aj[i] += x[ix] * t1 + y[iy] * t2;
                    ix += incx; iy += incy;
                }
                aj[j] = (R)crealq(aj[j]) + (R)crealq(x[jx] * t1 + y[jy] * t2);
            } else {
                aj[j] = (R)crealq(aj[j]);
            }
            jx += incx; jy += incy;
