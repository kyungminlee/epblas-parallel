/*
 * xhemm — kind16 (COMPLEX(KIND=16)) port of OpenBLAS ZHEMM.
 *
 ...
 *   - kernel/generic/
 *       zhemm_utcopy_2.c         → qblas_yhemm_ucopy  (shared via common/)
 *       zhemm_ltcopy_2.c         → qblas_yhemm_lcopy  (shared via common/)
 *
 ...
 * from common/qblas_l3_complex.c. The non-Hermitian factor (B in
 * SIDE=L, A in SIDE=R) goes through the standard packer with conj=0.
 *
 * Fortran ABI:
 *   subroutine xhemm(side, uplo, m, n, alpha, a, lda, b, ldb,
 *                    beta, c, ldc)
            ...
            for (int ls = 0; ls < K; ls += KC) {
...
#endif
                {
                    if (side == 'L') {
                         ...
                         * (posX, posY) — see qblas_l3_complex.h). */
                        if (uplo == 'U') {
                            qblas_yhemm_ucopy_oc(pb, jb,
                                B_eff, ldb_eff, js, ls, Bp);
                        } else {
                            qblas_yhemm_lcopy_oc(pb, jb,
                                B_eff, ldb_eff, js, ls, Bp);
                        }
                    }
                }
                ...
                for (int is = m_lo; is < m_hi; is += MC) {
                    ...
                    if (side == 'L') {
                        if (uplo == 'U') {
                            qblas_yhemm_ucopy(pb, min_i,
                                A_eff, lda_eff, is, ls, Ap);
                        } else {
                            qblas_yhemm_lcopy(pb, min_i,
                                A_eff, lda_eff, is, ls, Ap);
                        }
                    } else {
