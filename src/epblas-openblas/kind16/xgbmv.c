/*
 * xgbmv — kind16 port of OpenBLAS zgbmv (interface/zgbmv.c +
                          ...
                          const C *x, C *y_priv)
{
    for (ptrdiff_t j = j_lo; j < j_hi; ++j) {
        ...
        ptrdiff_t row_off = ku - j;
        for (ptrdiff_t i = i_lo; i <= i_hi; ++i)
            s += conjq(col[row_off + i]) * x[i];
        y_priv[j] += s;
    }
}

void xgbmv_(const char *TRANS, const int *M, const int *N,
            ...
            const C *BETA, C *y, const int *INCY)
{
    ...
    if (m == 0 || n == 0 || (alpha == 0.0Q && beta == 1.0Q)) return;
    ...
    if (beta != 1.0Q) {
        ...
        ptrdiff_t iy = 0;
        if (beta == 0.0Q) for (ptrdiff_t i = 0; i < leny; ++i) { y[iy] = 0.0Q; iy += absy; }
        ...
        if (incx == 1) {
            ...
            for (ptrdiff_t j = 0; j < n; ++j) {
                ...
                if (noconj) {
                    ...
                    for (ptrdiff_t i = i_lo; i <= i_hi; ++i) temp += conjq(col[off + i]) * x[i];
                }
                y[jy] += alpha * temp;
                jy += incy;
            }
            ...
            for (ptrdiff_t j = 0; j < n; ++j) {
                ...
                if (noconj) {
                    ...
                    for (ptrdiff_t i = i_lo; i <= i_hi; ++i) {
                        temp += conjq(col[off + i]) * x[ix]; ix += incx;
                    }
                }
                y[jy] += alpha * temp;
                jy += incy;
