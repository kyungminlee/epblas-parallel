/*
 * wtrsm — kind10 (COMPLEX(KIND=10)) port of OpenBLAS ZTRSM.
 *
...
{
    if (!trans) {
        mblas_ytrsm_iltcopy(m, n, a, lda, offset, bp, unit, conj);
    } else {
        mblas_ytrsm_iuncopy(m, n, a, lda, offset, bp, unit, conj);
    }
}
...
{
    if (!trans) {
        mblas_ytrsm_iutcopy(m, n, a, lda, offset, bp, unit, conj);
    } else {
        mblas_ytrsm_ilncopy(m, n, a, lda, offset, bp, unit, conj);
    }
}
        ...
        if (forward) {
            for (int ls = 0; ls < m; ls += KC) {
                ...
                for (int is = ls + min_l; is < m; is += MC) {
                    ...
                    if (min_i > MC) min_i = MC;
                    if (!trans) {
                        mblas_ygemm_tcopy(min_l, min_i, conj,
                                          &a[(size_t)is * 2 + (size_t)ls * lda * 2], lda, Ap);
                    } else {
                        mblas_ygemm_ncopy(min_l, min_i, conj,
                                          &a[(size_t)ls * 2 + (size_t)is * lda * 2], lda, Ap);
                    }
                    mblas_ygemm_kernel(min_i, min_j, min_l, dm1r, dm1i,
                                       Ap, Bp,
                                       &b[(size_t)is * 2 + (size_t)js * ldb * 2], ldb);
                }
            }
        ...
        for (int js = 0; js < N; js += NC) {
            ...
            for (int ls = 0; ls < js; ls += KC) {
                ...
                for (int jjs = js; jjs < js + min_j; jjs += NR) {
                    ...
                    if (min_jj > NR) min_jj = NR;
                    if (!trans) {
                        mblas_ygemm_ncopy(min_l, min_jj, conj,
                                          &a[(size_t)ls * 2 + (size_t)jjs * lda * 2], lda,
                                          sb + (size_t)min_l * (jjs - js) * 2);
                    } else {
                        mblas_ygemm_tcopy(min_l, min_jj, conj,
                                          &a[(size_t)jjs * 2 + (size_t)ls * lda * 2], lda,
                                          sb + (size_t)min_l * (jjs - js) * 2);
                    }
                    mblas_ygemm_kernel(min_i, min_jj, min_l, dm1r, dm1i,
            ...
            for (int ls = js; ls < js + min_j; ls += KC) {
                ...
                for (int is = min_i; is < m_band; is += MC) {
                    ...
                    if (min_j - min_l + js - ls > 0) {
                        mblas_ygemm_kernel(min_i, min_j - min_l + js - ls, min_l, dm1r, dm1i,
            ...
            for (int ls = start_ls; ls >= js - min_j; ls -= KC) {
                ...
                for (int is = min_i; is < m_band; is += MC) {
                    ...
                    mblas_ytrsm_kernel(/*left=*/0, kt,
                                       ...
                                       0);
                    if (min_j - js + ls > 0) {
                        mblas_ygemm_kernel(min_i, min_j - js + ls, min_l, dm1r, dm1i,
                                           sa, sb,
                                           &b[(size_t)(m_lo + is) * 2 + (size_t)(js - min_j) * ldb * 2], ldb);
                    }
                }
...
extern "C" void wtrsm_(
    ...
    const int *m_, const int *n_, const T *alpha_,   /* alpha is 2 multifloats::float64x2s */
    ...
    T *b, const int *ldb_)
{
    ...
    if (alpha_r == 0.0 && alpha_i == 0.0) {
        mblas_ygemm_beta((ptrdiff_t)M, (ptrdiff_t)N, alpha_r, alpha_i, b, (ptrdiff_t)ldb);
        return;
    }
    ...
    /* 2x the size for complex (2 multifloats::float64x2s per element). */
    ...
    if (!Ap_arr || !Bp_arr) {
        ...
        fprintf(stderr, "wtrsm: pack buffer allocation failed\n");
        abort();
    }
    int alloc_ok = 1;
