/*
 * mtrsv — kind10 port of OpenBLAS dtrsv.  Triangular solve.
 *
 *   x := inv(op(A)) * x      where op(A) = A or A^T
 *
 * Loop directions match Fortran reference mtrsv.f line-for-line.
 *
 * Fortran ABI:  subroutine mtrsv(uplo, trans, diag, n, a, lda, x, incx)
 */
...
extern "C" void mtrsv_(const char *UPLO, const char *TRANS, const char *DIAG,
