/*
 * wtbsv — kind10 port of OpenBLAS ztbsv.  Complex banded triangular solve.
 *
 *   x := inv(op(A)) * x   (op = A, A^T, or A^H).  Band storage matches etbsv.
 *
 * Fortran ABI:  subroutine wtbsv(uplo, trans, diag, n, k, a, lda, x, incx)
 */
...
extern "C" void wtbsv_(const char *UPLO, const char *TRANS, const char *DIAG,
