/*
 * wtbmv — kind10 port of OpenBLAS ztbmv (interface/tbmv.c +
 ...
 *
 * Fortran ABI:  subroutine wtbmv(uplo, trans, diag, n, k, a, lda, x, incx)
 */
        ...
        if (upper) {
            if (length > 0) {
                if (!trans) {
                    ...
                    else       for (ptrdiff_t j = 0; j < length; ++j) s += std::conj(coff[j]) * xoff[j];
                    y[i] += s;
                }
            }
            if (nounit) y[i] += (conj && trans) ? std::conj(col[k]) * x[i] : col[k] * x[i];
            else        y[i] += x[i];
        } else {
            if (nounit) y[i] += (conj && trans) ? std::conj(col[0]) * x[i] : col[0] * x[i];
            else        y[i] += x[i];
            if (length > 0) {
                if (!trans) {
                    C xi = x[i];
                    ...
                    else       for (ptrdiff_t j = 0; j < length; ++j) s += std::conj(col[1 + j]) * xoff[j];
                    y[i] += s;
                }
            }
        }
...
extern "C" void wtbmv_(const char *UPLO, const char *TRANS, const char *DIAG,
...
#define CONJIF(z) (noconj ? (z) : std::conj(z))
