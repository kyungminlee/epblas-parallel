/*
 * wherk — kind10 (COMPLEX(KIND=10)) Hermitian rank-k update.
 *
 ...
 *                                → mblas_yherk_kernel_{u,l}
 *                                  (in common/mblas_l3_complex.c)
 *   - driver/level3/zherk_beta.c → mblas_yherk_beta_{u,l}
 *                                  (in common/mblas_l3_complex.c)
 *
 ...
 * Our shared mblas_ygemm_kernel is NN-only (matching ygemm/ysymm —
 * conjugation absorbed at pack time), so we set `conj` on the side the
 * upstream kernel would have conjugated:
 *   - TRANS='N': Bp packer uses conj=1 (caller of wherk path passes A
 ...
 *   - alpha, beta are REAL(KIND=10): one multifloats::float64x2 each
        ...
        for (int js = 0; js < N; js += NC) {
            ...
            for (int ls = 0; ls < K; ls += KC) {
...
#endif
                {
                    ...
                    if (trans == 'N') {
                        mblas_ygemm_tcopy(pb, jb, conj_b,
                            &a[((size_t)ls * lda + js) * 2], lda, Bp);
                    } else {
                        mblas_ygemm_ncopy(pb, jb, conj_b,
                ...
                for (int is = m_lo_eff; is < m_hi_eff; is += MC) {
                    ...
                    if (trans == 'N') {
                        mblas_ygemm_tcopy(pb, min_i, conj_a,
                            &a[((size_t)ls * lda + is) * 2], lda, Ap);
                    } else {
                        mblas_ygemm_ncopy(pb, min_i, conj_a,
                            &a[((size_t)is * lda + ls) * 2], lda, Ap);
                    }

                    if (uplo == 'U') {
                        mblas_yherk_kernel_u(min_i, jb, pb, alphar,
                            Ap, Bp,
                            &c[((size_t)js * ldc + is) * 2], ldc,
                            (ptrdiff_t)(is - js));
                    } else {
                        mblas_yherk_kernel_l(min_i, jb, pb, alphar,
                            Ap, Bp,
                            &c[((size_t)js * ldc + is) * 2], ldc,
                            (ptrdiff_t)(is - js));
                    }
