/*
 * wtpsv — kind10 port of OpenBLAS ztpsv.  Complex packed triangular solve.
 *
 *   x := inv(op(A)) * x  (op = A, A^T, A^H).  Packed storage matches etpsv.
 *
 * Fortran ABI:  subroutine wtpsv(uplo, trans, diag, n, ap, x, incx)
 */
...
extern "C" void wtpsv_(const char *UPLO, const char *TRANS, const char *DIAG,
