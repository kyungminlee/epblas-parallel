/*
 * wtpmv — kind10 port of OpenBLAS ztpmv (interface/tpmv.c +
 ...
 *   - 'C' (conj-trans) wraps ap reads with std::conj().
 *
 * Fortran ABI:  subroutine wtpmv(uplo, trans, diag, n, ap, x, incx)
 */
...
static void tpmv_kernel_T(int upper, int nounit, int conj, ptrdiff_t n,
                          ...
                          const C *ap, const C *x, C *y)
{
#define APV(k) (conj ? std::conj(ap[k]) : ap[k])
    if (upper) {
        for (ptrdiff_t j = m_from; j < m_to; ++j) {
            size_t cs = col_start_U(j);
            C s = C(0.0, 0.0);
...
extern "C" void wtpmv_(const char *UPLO, const char *TRANS, const char *DIAG,
