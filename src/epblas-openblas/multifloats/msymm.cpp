 ...
 *   - kernel/generic/
 *       symm_ucopy_2.c           → mblas_esymm_ucopy  (shared via common/)
 *       symm_lcopy_2.c           → mblas_esymm_lcopy  (shared via common/)
 *
 * The microkernel + standard GEMM packers (mblas_egemm_kernel /
 * mblas_egemm_ncopy / mblas_egemm_tcopy / mblas_egemm_beta) are reused
 * from the shared common/mblas_l3_real.c — SYMM only changes WHICH
    ...
    T *c, const int *ldc_)
{
    ...
    /* Beta pre-pass. */
    mblas_egemm_beta((ptrdiff_t)M, (ptrdiff_t)N, beta, c, (ptrdiff_t)(*ldc_));
            ...
            for (int ls = 0; ls < K; ls += KC) {
...
#endif
                {
                    if (side == 'L') {
                        ...
                        if (uplo == 'U') {
                            mblas_esymm_ucopy(pb, jb,
                                B_eff, ldb_eff, js, ls, Bp);
                        } else {
                            mblas_esymm_lcopy(pb, jb,
                                B_eff, ldb_eff, js, ls, Bp);
                        }
                    }
                }
                ...
                for (int is = m_lo; is < m_hi; is += MC) {
                    ...
                    if (side == 'L') {
                        if (uplo == 'U') {
                            mblas_esymm_ucopy(pb, min_i,
                                A_eff, lda_eff, is, ls, Ap);
                        } else {
                            mblas_esymm_lcopy(pb, min_i,
                                A_eff, lda_eff, is, ls, Ap);
                        }
                    } else {
                    ...
                    mblas_egemm_kernel(min_i, jb, pb, alpha,
                                       Ap, Bp,
