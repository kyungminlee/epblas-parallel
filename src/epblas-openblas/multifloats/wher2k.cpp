/*
 * wher2k — kind10 (COMPLEX(KIND=10)) Hermitian rank-2k update.
 *
 ...
 *                                     conj(alpha)·B·A^H)
 *   - driver/level3/zher2k_kernel.c → mblas_yher2k_kernel_{u,l}
 *                                     (in common/mblas_l3_complex.c)
 *   - driver/level3/syrk_beta (HER2K variant): same shape as
 *                                     zherk_beta.c (real beta,
 *                                     unconditional diag imag = 0),
 *                                     so we reuse mblas_yherk_beta_{u,l}
 *                                     directly.
 *
 ...
 *
 * The diagonal mirror lives inside mblas_yher2k_kernel — it computes
 ...
 *   - beta  is REAL(KIND=10): 1 multifloats::float64x2
    ...
    size_t uplo_len, size_t trans_len)
{
    ...
    if (uplo == 'U') mblas_yherk_beta_u((ptrdiff_t)N, beta_r,
                                        c, (ptrdiff_t)ldc);
    else             mblas_yherk_beta_l((ptrdiff_t)N, beta_r,
                                        c, (ptrdiff_t)ldc);

    if (K == 0 || (alphar == 0.0 && alphai == 0.0)) return;
    ...
    T *Bp_A = (T *)aligned_alloc(64, (bp_bytes + 63) & ~(size_t)63);
    T *Bp_B = (T *)aligned_alloc(64, (bp_bytes + 63) & ~(size_t)63);
    if (!Bp_A || !Bp_B) { free(Bp_A); free(Bp_B); return; }

    T **Ap_A_arr = (T **)calloc((size_t)nthreads, sizeof(T *));
    T **Ap_B_arr = (T **)calloc((size_t)nthreads, sizeof(T *));
    if (!Ap_A_arr || !Ap_B_arr) {
        free(Ap_A_arr); free(Ap_B_arr); free(Bp_A); free(Bp_B); return;
    }
    int alloc_ok = 1;
    for (int t = 0; t < nthreads; ++t) {
        Ap_A_arr[t] = (T *)aligned_alloc(64, (ap_bytes + 63) & ~(size_t)63);
        Ap_B_arr[t] = (T *)aligned_alloc(64, (ap_bytes + 63) & ~(size_t)63);
        ...
        for (int js = 0; js < N; js += NC) {
            ...
            for (int ls = 0; ls < K; ls += KC) {
...
#endif
                {
                    ...
                    if (trans == 'N') {
                        mblas_ygemm_tcopy(pb, jb, conj_b_pack,
                            &a[((size_t)ls * lda + js) * 2], lda, Bp_A);
                        mblas_ygemm_tcopy(pb, jb, conj_b_pack,
                            &b[((size_t)ls * ldb + js) * 2], ldb, Bp_B);
                    } else {
                        mblas_ygemm_ncopy(pb, jb, conj_b_pack,
                ...
                for (int is = m_lo_eff; is < m_hi_eff; is += MC) {
                    ...
                    if (trans == 'N') {
                        mblas_ygemm_tcopy(pb, min_i, conj_a_pack,
                            &a[((size_t)ls * lda + is) * 2], lda, Ap_A);
                        mblas_ygemm_tcopy(pb, min_i, conj_a_pack,
                            &b[((size_t)ls * ldb + is) * 2], ldb, Ap_B);
                    } else {
                        mblas_ygemm_ncopy(pb, min_i, conj_a_pack,
                    ...
                    if (uplo == 'U') {
                        mblas_yher2k_kernel_u(min_i, jb, pb, alphar, alphai,
                            Ap_A, Bp_B,
                        ...
                        mblas_yher2k_kernel_l(min_i, jb, pb, alphar, alphai,
                            Ap_A, Bp_B,
                            &c[((size_t)js * ldc + is) * 2], ldc,
                            (ptrdiff_t)(is - js), 1);
                        mblas_yher2k_kernel_l(min_i, jb, pb, alphar, -alphai,
