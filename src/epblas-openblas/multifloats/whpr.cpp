/*
 * whpr — kind10 port of OpenBLAS zhpr.  Hermitian packed rank-1 update.
 *
 ...
 * forward-mapped.
 *
 * Fortran ABI:  subroutine whpr(uplo, n, alpha, x, incx, ap)
 */
...
extern "C" void whpr_(const char *UPLO, const int *N, const R *ALPHA,
                ...
                #pragma omp parallel num_threads(num_cpu)
                {
                    ...
                    if (upper) {
                        for (ptrdiff_t j = m_from; j < m_to; ++j) {
                            ...
                            if (x[j] != C(0.0, 0.0)) {
                                C temp = (C)alpha * std::conj(x[j]);
                                for (ptrdiff_t i = 0; i < j; ++i) aj[i] += x[i] * temp;
                                aj[j] = (R)(aj[j]).real() + (R)(x[j] * temp).real();
                            } else {
                                aj[j] = (R)(aj[j]).real();
                            }
                        }
                        ...
                        for (ptrdiff_t j = m_from; j < m_to; ++j) {
                            ...
                            if (x[j] != C(0.0, 0.0)) {
                                C temp = (C)alpha * std::conj(x[j]);
                                aj[j] = (R)(aj[j]).real() + (R)(temp * x[j]).real();
        ...
        if (upper) {
            for (ptrdiff_t j = 0; j < n; ++j) {
                ...
                if (x[jx] != C(0.0, 0.0)) {
                    ...
                    aj[j] = (R)(aj[j]).real() + (R)(x[jx] * temp).real();
                } else {
                    aj[j] = (R)(aj[j]).real();
                }
                jx += incx;
            ...
            for (ptrdiff_t j = 0; j < n; ++j) {
                ...
                if (x[jx] != C(0.0, 0.0)) {
                    C temp = (C)alpha * std::conj(x[jx]);
                    aj[j] = (R)(aj[j]).real() + (R)(temp * x[jx]).real();
                    ptrdiff_t ix = jx;
