/*
 * wdotc — multifloats DD port of OpenBLAS zdotc.  Returns sum(conj(x)*y).
 */
#include <stddef.h>
#include <complex>
                                  ...
                                  const C *y, ptrdiff_t incy)
{
    C s = C(0.0, 0.0);
    if (incx == 1 && incy == 1) {
        for (ptrdiff_t i = 0; i < n; ++i) s += std::conj(x[i]) * y[i];
        return s;
    }
    for (ptrdiff_t i = 0; i < n; ++i) s += std::conj(x[i*incx]) * y[i*incy];
    return s;
}

extern "C" C wdotc_(const int *N, const C *x, const int *INCX,
