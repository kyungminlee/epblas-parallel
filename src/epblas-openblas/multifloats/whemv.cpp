/*
 * whemv — kind10 port of OpenBLAS zhemv.  Hermitian MV.
 *
 ...
 *
 * Fortran ABI:  subroutine whemv(uplo, n, alpha, a, lda, x, incx, beta, y, incy)
 */
...
extern "C" void whemv_(const char *UPLO, const int *N, const C *ALPHA,
            ...
            const C *BETA, C *y, const int *INCY)
{
    ...
    if (n == 0 || (alpha == C(0.0, 0.0) && beta == C(1.0, 0.0))) return;
    ...
    if (beta != C(1.0, 0.0)) {
        ...
        ptrdiff_t iy = 0;
        if (beta == C(0.0, 0.0)) for (ptrdiff_t i = 0; i < n; ++i) { y[iy] = C(0.0, 0.0); iy += absy; }
                        ...
                        if (upper) {
                            for (ptrdiff_t j = m_from; j < m_to; ++j) {
                                ...
                                for (ptrdiff_t i = 0; i < j; ++i) {
                                    slot[i] += t1 * aj[i];
                                    t2      += std::conj(aj[i]) * x[i];
                                }
                                slot[j] += t1 * (R)(aj[j]).real() + t2;
                            }
                        } else {
                            for (ptrdiff_t j = m_from; j < m_to; ++j) {
                                C t1 = x[j];
    ...
    if (upper) {
        for (ptrdiff_t j = 0; j < n; ++j) {
            ...
            for (ptrdiff_t i = 0; i < j; ++i) {
                y[iy] += t1 * A_(i, j);
                t2    += std::conj(A_(i, j)) * x[ix];
                ix += incx; iy += incy;
            }
            y[jy] += t1 * (R)(A_(j, j)).real() + alpha * t2;
            jx += incx; jy += incy;
        }
    } else {
        for (ptrdiff_t j = 0; j < n; ++j) {
