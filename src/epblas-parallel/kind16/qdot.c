...
static TR qdot_kernel(ptrdiff_t n, const TR *x, ptrdiff_t incx, const TR *y, ptrdiff_t incy)
{
    TR s0 = 0.0Q, s1 = 0.0Q;
    if (incx == 1 && incy == 1) {
        TR s2 = 0.0Q, s3 = 0.0Q, s4 = 0.0Q;
        ptrdiff_t i = 0;
        for (; i + 4 < n; i += 5) {
            s0 += x[i    ] * y[i    ];
            s1 += x[i + 1] * y[i + 1];
...
__attribute__((noinline)) static bool qdot_omp(ptrdiff_t n, const TR *x, const TR *y, TR *out)
{
    if (n <= QDOT_OMP_MIN || !blas_omp_should_thread())
        return 0;
    ptrdiff_t nthreads = blas_omp_max_threads();
    if (nthreads > QDOT_MAX_CPUS) nthreads = QDOT_MAX_CPUS;
    TR partial[QDOT_MAX_CPUS] = {0};
...
static TR qdot_core(ptrdiff_t n, const TR *x, ptrdiff_t incx,
