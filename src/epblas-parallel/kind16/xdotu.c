...
static TC xdotu_kernel(ptrdiff_t n, const TC *x, ptrdiff_t incx, const TC *y, ptrdiff_t incy)
{
    TC s = 0;
    if (incx == 1 && incy == 1) {
...
__attribute__((noinline)) static bool xdotu_omp(ptrdiff_t n, const TC *x, const TC *y, TC *out)
{
    if (n <= XDOTU_OMP_MIN || !blas_omp_should_thread())
        return 0;
    ptrdiff_t nthreads = blas_omp_max_threads();
    if (nthreads > XDOTU_MAX_CPUS) nthreads = XDOTU_MAX_CPUS;
    TC partial[XDOTU_MAX_CPUS] = {0};
...
static TC xdotu_core(ptrdiff_t n, const TC *x, ptrdiff_t incx,
