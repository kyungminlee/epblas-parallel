    ...
    TR *restrict ap)
{
    ...
    if (would_thread) {
        ...
        enum { QSPR2_STACK_N = 256 };
        TR stackbuf[2 * QSPR2_STACK_N];
        _Static_assert(2 * QSPR2_STACK_N * sizeof(TR) <= sizeof(stackbuf),
                       "qspr2 stack-gather threshold exceeds stackbuf");
        TR *heap = NULL;
        TR *xc, *yc;
        if (n <= QSPR2_STACK_N) {
