...
static ptrdiff_t iqamax_kernel(ptrdiff_t n, const T *x, T *bv_out)
{
    ptrdiff_t best = 0;
    T bv = fabsq(x[0]);
...
static ptrdiff_t iqamax_core(ptrdiff_t n, const T *x, ptrdiff_t incx)
{
...
#endif
    ptrdiff_t best = 1; T bv = fabsq(x[0]); ptrdiff_t ix = incx;
    for (ptrdiff_t i = 2; i <= n; ++i) { T v = fabsq(x[ix]); if (v > bv) { bv = v; best = i; } ix += incx; }
    return best;
}

EPBLAS_FACADE_IAMAX(iqamax, T)
