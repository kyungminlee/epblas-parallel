 ...
 * C := alpha·(A^T·B + B^T·A) + beta·C over the UPLO triangle of output column
 * j. A,B are K×N so all dot operands are unit-stride over the K axis. beta is
 * FUSED into the store (no separate caller prescale — the yherk 9140682
 * shape): beta·C(i,j) is rounded to fp128 either way and IEEE add commutes,
 * so the fused form is bit-identical to prescale-then-+=; beta∈{0,1} skip
 * the multiply exactly as qsyrk_beta_{u,l} did. The beta case split is
 * resolved to an int ONCE per column — a per-element `beta == 0.0Q` would be
 * a libgcc __eqtf2 call inside the hot loop. Each C(i,j) is accumulated in a
 * register and written once — packing has nothing to save here, so the clean
 * unpacked loop matches the reference. */
void qsyr2k_trans_col(ptrdiff_t j, char UPLO, ptrdiff_t n, ptrdiff_t k,
                      TR alpha, TR beta, const TR *a, ptrdiff_t lda,
