/*
 * qsyr2k_serial.c — kind16 (REAL(KIND=16) / __float128) symmetric rank-2k,
 * serial core.
 *
 * Owns ALL the numerics shared by the serial and parallel entries: the uplo
 * decode and the per-column compute core (declared in qsyr2k_kernel.h), plus
 * the public `qsyr2k_serial_` Fortran entry. No OpenMP anywhere on this call
 * path — safe to invoke from inside another function's `#pragma omp parallel`
 * region.
 *
 * Both qsyr2k_serial_ and the parallel qsyr2k_ drive numerics through
 * qsyr2k_core, so the two paths are bitwise-identical.
 */
