    ...
    T *a, const int *lda_)
{
    ...
    if (incx == 1 && incy == 1) {
         ...
         * column loop reads it contiguously; mirrors ob zgerc_thread.c. Columns
        ...
        for (int j = 0; j < N; ++j) {
            ...
            if (!cdd_iszero(yj)) {
                const T t = cmul(alpha, yj);
                T *aj = &A_(0, j);
                if (x_unit) { for (int i = 0; i < M; ++i) aj[i] = cadd(aj[i], cmul(t, xp[i])); }
                else { int ix = ix0; for (int i = 0; i < M; ++i) { aj[i] = cadd(aj[i], cmul(t, x[ix])); ix += incx; } }
            }
        }
        std::free(x_buf);
    }
