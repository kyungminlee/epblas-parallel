...
static void wgerc_contig_simd(std::ptrdiff_t m, std::ptrdiff_t n, TC alpha, TC *a, std::size_t lda,
 ...
 * N. Runtime dispatch: SIMD column-AXPY (cmul/cadd) on Haswell+, scalar (always
...
static void wgerc_contig(std::ptrdiff_t m, std::ptrdiff_t n, TC alpha, TC *a, std::size_t lda,
                         const TC *x, const TC *y)
{
#ifdef MBLAS_SIMD_DD
    if (mf_have_avx2_fma()) { wgerc_contig_simd(m, n, alpha, a, lda, x, y); return; }
#endif
#ifdef _OPENMP
        const bool use_omp = (n >= WGERC_OMP_MIN && blas_omp_available());
        #pragma omp parallel for if(use_omp) schedule(static)
