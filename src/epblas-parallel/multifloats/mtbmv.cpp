                              ...
                              const TR *a, std::ptrdiff_t lda, TR *x, std::ptrdiff_t incx)
{
    ...
    for (std::ptrdiff_t i = 0; i < n; ++i) { const TR v = x[(std::ptrdiff_t)i * incx]; xh[i] = v.limbs[0]; xl[i] = v.limbs[1]; }
    ...
    if (upper) {
        for (std::ptrdiff_t j = 0; j < n; ++j) {
            ...
            const TR xj{xh[j], xl[j]};
            for (; i < j; ++i) { TR xi{xh[i], xl[i]}; xi = xi + xj * col[off + i]; xh[i] = xi.limbs[0]; xl[i] = xi.limbs[1]; }
            if (nounit) { TR d{xh[j], xl[j]}; d = d * col[k]; xh[j] = d.limbs[0]; xl[j] = d.limbs[1]; }
        }
    } else {
        for (std::ptrdiff_t j = n - 1; j >= 0; --j) {
            if (xh[j] == 0.0 && xl[j] == 0.0) continue;
            ...
            const TR xj{xh[j], xl[j]};
            for (; i <= i_hi; ++i) { TR xi{xh[i], xl[i]}; xi = xi + xj * col[off + i]; xh[i] = xi.limbs[0]; xl[i] = xi.limbs[1]; }
            if (nounit) { TR d{xh[j], xl[j]}; d = d * col[0]; xh[j] = d.limbs[0]; xl[j] = d.limbs[1]; }
        }
    }

    for (std::ptrdiff_t i = 0; i < n; ++i) x[(std::ptrdiff_t)i * incx] = TR{xh[i], xl[i]};
...
static void mtbmv_rowgather_t_soa(bool upper, bool nounit, std::ptrdiff_t n, std::ptrdiff_t k,
                                  std::ptrdiff_t lo, std::ptrdiff_t hi, const TR *a, std::ptrdiff_t lda,
    ...
    if (upper) {
        while (r < hi) {
            if (r >= k && r + 4 <= hi) {                 /* full band: llen == k */
                ...
                const TR *base = &A_(0, r);
                const std::ptrdiff_t llen = (r < k) ? r : k;
                TR s = nounit ? base[k] * TR{xh[r], xl[r]} : TR{xh[r], xl[r]};
                for (std::ptrdiff_t d = 1; d <= llen; ++d) { const TR xv{xh[r - d], xl[r - d]}; s = s + base[k - d] * xv; }
        ...
        while (r < hi) {
            if (r + 3 <= n - 1 - k && r + 4 <= hi) {     /* full band: rlen == k */
                ...
                TR s = nounit ? base[0] * TR{xh[r], xl[r]} : TR{xh[r], xl[r]};
                for (std::ptrdiff_t d = 1; d <= rlen; ++d) { const TR xv{xh[r + d], xl[r + d]}; s = s + base[d] * xv; }
                yh[r] = s.limbs[0]; yl[r] = s.limbs[1];
                ++r;
            }
        }
                            ...
                            const TR *a, std::ptrdiff_t lda, TR *x, std::ptrdiff_t incx)
{
    ...
    for (std::ptrdiff_t i = 0; i < n; ++i) x[(std::ptrdiff_t)i * incx] = TR{yh[i], yl[i]};
...
static void mtbmv_rowgather_t(bool upper, bool nounit,
                              ...
                              const TR *a, std::size_t lda,
                              const TR *xin, TR *y)
{
    for (std::ptrdiff_t r = lo; r < hi; ++r) {
        const TR *base = &A_(0, r);
        const TR diagc = upper ? base[k] : base[0];
        TR s = nounit ? diagc * xin[r] : xin[r];
        if (upper) {
            const std::ptrdiff_t llen = (r < k) ? r : k;
...
static void mtbmv_colscatter(bool upper, bool nounit, std::ptrdiff_t n, std::ptrdiff_t k,
                             std::ptrdiff_t lo, std::ptrdiff_t hi, const TR *a, std::size_t lda,
                             const TR *xin, TR *y)
{
    if (upper) {
        ...
        for (std::ptrdiff_t j = lo; j < jmax; ++j) {
            ...
            const TR *col = &A_(L + i_lo, j);             /* contiguous in i */
        ...
        for (std::ptrdiff_t j = hi - 1; j >= jmin; --j) {
            ...
            const TR *col = &A_(i_lo - j, j);             /* A(i,j)=A_(i-j,j), contiguous */
            for (std::ptrdiff_t i = i_lo; i < i_hi; ++i) y[i] = y[i] + tmp * (*col++);
            if (j >= lo && j < hi)                       /* diagonal seed */
                                 ...
                                 double *yh, double *yl)
{
    if (upper) {
        ...
        for (std::ptrdiff_t j = lo; j < jmax; ++j) {
            ...
            const TR tmp{xh[j], xl[j]};
            for (; i < i_hi; ++i) { TR yi{yh[i], yl[i]}; yi = yi + tmp * col[off + i]; yh[i] = yi.limbs[0]; yl[i] = yi.limbs[1]; }
            if (j >= lo && j < hi) {                      /* diagonal seed */
                if (nounit) { const TR d = tmp * col[k]; yh[j] = d.limbs[0]; yl[j] = d.limbs[1]; }
        ...
        for (std::ptrdiff_t j = hi - 1; j >= jmin; --j) {
            ...
            for (; i < i_hi; ++i) { TR yi{yh[i], yl[i]}; yi = yi + tmp * col[off + i]; yh[i] = yi.limbs[0]; yl[i] = yi.limbs[1]; }
            if (j >= lo && j < hi) {                      /* diagonal seed */
                if (nounit) { const TR d = tmp * col[0]; yh[j] = d.limbs[0]; yl[j] = d.limbs[1]; }
                else { yh[j] = xh[j]; yl[j] = xl[j]; }
            }
        }
    }
    ...
    const TR *a, std::size_t lda, TR *x, std::ptrdiff_t incx)
{
     ...
     * per-row column/d order keep it bit-exact. Alloc failure -> scalar below. */
    {
        ...
        if (xh && xl && yh && yl) {
            for (std::ptrdiff_t i = 0; i < n; ++i) {
                const TR v = x[(std::ptrdiff_t)i * incx];
            ...
            #pragma omp parallel num_threads(nthreads)
            {
                ...
                for (std::ptrdiff_t i = lo; i < hi; ++i)
                    x[(std::ptrdiff_t)i * incx] = TR{yh[i], yl[i]};
            }
            std::free(xh); std::free(xl); std::free(yh); std::free(yl);
            return true;
        }
    ...
    TR *xbuf = NULL;
    if (incx != 1) {
        xbuf = static_cast<TR *>(std::malloc((std::size_t)n * sizeof(TR)));
        if (!xbuf) return false;
        for (std::ptrdiff_t i = 0; i < n; ++i) xbuf[i] = x[(std::ptrdiff_t)i * incx];
        xptr = xbuf;
    }
    TR *y = static_cast<TR *>(std::malloc((std::size_t)n * sizeof(TR)));
