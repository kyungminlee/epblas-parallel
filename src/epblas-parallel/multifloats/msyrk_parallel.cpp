/*
 * msyrk_ — multifloats real (DD) symmetric rank-k update, public Fortran
 * entry. THREADING ORCHESTRATION ONLY: all the math lives in msyrk_serial.cpp
 * (SIMD + scalar diagonal kernels, block policy, the per-block worker and the
 * per-column triangle scaler), shared through msyrk_kernel.h.
 *
 *   C := α·A·Aᵀ + β·C   (TR='N')      C := α·Aᵀ·A + β·C   (TR='T'/'C')
 *
 ...
 * scale (schedule(static)). Delegates to msyrk_serial when nested.
 */
    ...
    std::size_t uplo_len, std::size_t trans_len)
{
#ifdef _OPENMP
    if (omp_in_parallel()) {
        msyrk_serial(uplo, trans, n_, k_, alpha_, a, lda_, beta_, c, ldc_,
                     uplo_len, trans_len);
        return;
    }
#endif
    ...
    const bool use_omp = (N >= MSYRK_OMP_MIN && nt > 1);
