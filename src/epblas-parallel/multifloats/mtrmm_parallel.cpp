...
#ifdef _OPENMP
    if (omp_in_parallel()) {
        mtrmm_serial(side, uplo, transa, diag, m_, n_, alpha_, a, lda_,
...
#ifdef _OPENMP
        const int use_omp = (N >= MTRMM_OMP_MIN && blas_omp_max_threads() > 1);
        if (use_omp) {
            #pragma omp parallel
            {
...
#ifdef _OPENMP
        const int use_omp = (M >= MTRMM_OMP_MIN && blas_omp_max_threads() > 1);
