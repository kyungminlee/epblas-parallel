...
static void erot_unit(ptrdiff_t n, TR c, TR s, TR *x, TR *y)
{
    ptrdiff_t i, n1 = n & -4;
    for (i = 0; i < n1; i += 4) {
        TR x0 = x[i+0], y0 = y[i+0];
        x[i+0] = c * x0 + s * y0;  y[i+0] = c * y0 - s * x0;
        TR x1 = x[i+1], y1 = y[i+1];
        x[i+1] = c * x1 + s * y1;  y[i+1] = c * y1 - s * x1;
        TR x2 = x[i+2], y2 = y[i+2];
        x[i+2] = c * x2 + s * y2;  y[i+2] = c * y2 - s * x2;
        TR x3 = x[i+3], y3 = y[i+3];
...
static bool erot_omp(ptrdiff_t n, TR c, TR s, TR *x, TR *y)
{
...
static void erot_core(ptrdiff_t n, TR *x, ptrdiff_t incx, TR *y, ptrdiff_t incy,
