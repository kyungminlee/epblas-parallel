    ...
    TR *restrict x, ptrdiff_t incx)
{
     ...
     * beats both OpenBLAS and the gfortran reference by ~35% serial where
     * the strided walk only ties them; the O(n) gather/scatter is noise
     * against the O(n²/2) solve. Bit-identical: the contiguous substitution
     * applies the same per-element operations in the same order as the
     * strided walk. Trans keeps the direct strided walk (no contiguous
     * headroom over the references' strided handling — the ytrsv/etbsv
     * split), and that walk doubles as the alloc-failure fallback. Stack
     * scratch for the common small-N case avoids malloc latency. */
    if (incx != 1 && blas_up(trans) == 'N') {
        enum { ETRSV_STACK_N = 512 };
        TR stackbuf[ETRSV_STACK_N];
        TR *heap = NULL;
        TR *xc = (n <= ETRSV_STACK_N)
            ? stackbuf
        ...
        if (xc) {
            ...
            etrsv_core(uplo, trans, diag, n, a, lda, xc, 1);
            ix = kx0;
