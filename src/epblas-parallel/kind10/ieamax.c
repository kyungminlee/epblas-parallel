...
static ptrdiff_t ieamax_kernel(ptrdiff_t n, const T *x, T *bv_out)
{
    ptrdiff_t best = 0;
    T bv = fabsl(x[0]);
...
__attribute__((noinline)) static ptrdiff_t ieamax_omp(ptrdiff_t n, const T *x, ptrdiff_t *out)
{
...
#endif
    ptrdiff_t best = 1; T bv = fabsl(x[0]); ptrdiff_t ix = incx;
