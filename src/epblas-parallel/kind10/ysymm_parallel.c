    ...
    T *restrict c, ptrdiff_t ldc)
{
    ...
    if (alpha == ZERO) {
        ...
        const bool use_omp = (axis >= YSYMM_OMP_MIN && blas_omp_max_threads() > 1);
    ...
    if (SIDE == 'L' && M <= nb) {
#ifdef _OPENMP
        const bool use_omp = (N >= YSYMM_OMP_MIN && nthreads > 1);
        #pragma omp parallel for if(use_omp) schedule(static)
#endif
        for (ptrdiff_t j = 0; j < N; ++j)
    ...
    if (SIDE == 'L') {
        ...
        ptrdiff_t pw = nb;
#ifdef _OPENMP
        const bool use_omp = (M >= YSYMM_OMP_MIN && nthreads > 1);
