...
static TC ydotc_kernel(ptrdiff_t n, const TC *x, ptrdiff_t incx, const TC *y, ptrdiff_t incy)
{
    TC s = 0.0L;
    if (incx == 1 && incy == 1) {
        const TC *xe = x + n;
...
__attribute__((noinline)) static ptrdiff_t ydotc_omp(ptrdiff_t n, const TC *x, const TC *y, TC *out)
{
    if (n <= YDOTC_OMP_MIN || !blas_omp_should_thread())
        return 0;
    ptrdiff_t nthreads = blas_omp_max_threads();
    if (nthreads > YDOTC_MAX_CPUS) nthreads = YDOTC_MAX_CPUS;
    TC partial[YDOTC_MAX_CPUS] = {0};
...
static TC ydotc_core(ptrdiff_t n, const TC *x, ptrdiff_t incx,
