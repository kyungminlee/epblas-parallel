...
static void eswap_unit(ptrdiff_t n, TR *x, TR *y)
{
    ptrdiff_t i, m = n % 3;
    for (i = 0; i < m; ++i) { TR t = x[i]; x[i] = y[i]; y[i] = t; }
    for (; i < n; i += 3) {
        TR t0 = x[i+0]; x[i+0] = y[i+0]; y[i+0] = t0;
        TR t1 = x[i+1]; x[i+1] = y[i+1]; y[i+1] = t1;
        TR t2 = x[i+2]; x[i+2] = y[i+2]; y[i+2] = t2;
    }
}
...
static void eswap_slice(ptrdiff_t n, TR *x, TR *y)
{
    ptrdiff_t i, n1 = n & -4;
    for (i = 0; i < n1; i += 4) {
...
static bool eswap_omp(ptrdiff_t n, TR *x, TR *y)
{
...
static void eswap_core(ptrdiff_t n, TR *x, ptrdiff_t incx, TR *y, ptrdiff_t incy)
{
    if (n <= 0) return;
    if (incx == 1 && incy == 1) {
#ifdef _OPENMP
        ...
        for (ptrdiff_t i = 0; i < n; ++i) { TR t = x[ix]; x[ix] = y[iy]; y[iy] = t; ix += incx; iy += incy; }
    }
}

EPBLAS_FACADE_SWAP(eswap, TR)
