...
One dual_<routine>.{c,cpp} per routine under benchmark/dual/drivers/. Each driver
links the namespaced par_/ob_/mig_ archives (built by benchmark/dual/nsbuild.sh)
...
Implementation lives in benchmark/_perf_harness/:
...
Run from repo root:
    python3 benchmark/gen_dual_harnesses.py [--routines a,b,c]
...
OUTDIR = Path(__file__).resolve().parent / "dual" / "drivers"
