...
static void run_one(char trans, int M, int N, int incx, int incy,
    ...
    MFC *A  = PERF_ALLOC(MFC, (size_t)M * N);
    MFC *X  = PERF_ALLOC(MFC, lenx);
    MFC *Y  = PERF_ALLOC(MFC, leny);
    MFC *Yi = PERF_ALLOC(MFC, leny);
    PERF_FILL_C(MFC, A,  (size_t)M * N, 2);
    PERF_FILL_C(MFC, X,  lenx, 3);
    PERF_FILL_C(MFC, Yi, leny, 4);

    PERF_RESET(Y, Yi, leny, MFC);
    for (int r = 0; r < warmup; ++r) {
        wgemv_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1);          PERF_RESET(Y, Yi, leny, MFC);
        wgemv_migrated_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1); PERF_RESET(Y, Yi, leny, MFC);
    }

    double t_subject, t_mg;
    PERF_RESET(Y, Yi, leny, MFC);
    PERF_TIME(t_subject, iters, wgemv_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1));
    PERF_RESET(Y, Yi, leny, MFC);
    PERF_TIME(t_mg,      iters, wgemv_migrated_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1));
    ...
    PERF_EMIT("wgemv", key, N, iters, t_subject, t_mg);
