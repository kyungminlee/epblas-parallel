...
static void run_one(char side, char uplo, char trans, char diag,
    ...
    for (int r = 0; r < warmup; ++r) {
        wtrsm_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);          PERF_RESET(B, Bi, MNelt, MFC);
        wtrsm_migrated_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1); PERF_RESET(B, Bi, MNelt, MFC);
    }
    ...
    PERF_EMIT("wtrsm", key, N, iters, t_subject, t_mg);
