...
static void run_one(char trans, int M, int N, int incx, int incy,
    ...
    MFR *A  = PERF_ALLOC(MFR, (size_t)M * N);
    MFR *X  = PERF_ALLOC(MFR, lenx);
    MFR *Y  = PERF_ALLOC(MFR, leny);
    MFR *Yi = PERF_ALLOC(MFR, leny);
    PERF_FILL_R(MFR, A,  (size_t)M * N, 2);
    PERF_FILL_R(MFR, X,  lenx, 3);
    PERF_FILL_R(MFR, Yi, leny, 4);

    PERF_RESET(Y, Yi, leny, MFR);
    for (int r = 0; r < warmup; ++r) {
        mgemv_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1);          PERF_RESET(Y, Yi, leny, MFR);
        mgemv_migrated_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1); PERF_RESET(Y, Yi, leny, MFR);
    }

    double t_subject, t_mg;
    PERF_RESET(Y, Yi, leny, MFR);
    PERF_TIME(t_subject, iters, mgemv_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1));
    PERF_RESET(Y, Yi, leny, MFR);
