...
static void run_one(int N, int iters, int warmup) {
    ...
    PERF_FILL_C(MFC, Y, N, 1);
    for (int r2 = 0; r2 < warmup; ++r2) {
        r = wdotu_(&N, X, &one, Y, &one);          sink_T(&r);
        r = wdotu_migrated_(&N, X, &one, Y, &one); sink_T(&r);
    }
    double t_subject, t_mg;
    PERF_TIME(t_subject, iters, r = wdotu_(&N, X, &one, Y, &one); sink_T(&r));
    PERF_TIME(t_mg,      iters, r = wdotu_migrated_(&N, X, &one, Y, &one); sink_T(&r));
    PERF_EMIT("wdotu", "-", N, iters, t_subject, t_mg);
    free(X); free(Y);
}
