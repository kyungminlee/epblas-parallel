...
static void run_one(char uplo, int N, int incx, int incy, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        whpmv_(&uplo, &N, &alpha, AP, X, &incx, &beta, Y, &incy, 1);          PERF_RESET(Y, Yi, leny, MFC);
        whpmv_migrated_(&uplo, &N, &alpha, AP, X, &incx, &beta, Y, &incy, 1); PERF_RESET(Y, Yi, leny, MFC);
    }
    double t_subject, t_mg;
    PERF_RESET(Y, Yi, leny, MFC);
    PERF_TIME(t_subject, iters, whpmv_(&uplo, &N, &alpha, AP, X, &incx, &beta, Y, &incy, 1));
    PERF_RESET(Y, Yi, leny, MFC);
    PERF_TIME(t_mg,      iters, whpmv_migrated_(&uplo, &N, &alpha, AP, X, &incx, &beta, Y, &incy, 1));
    char key[24];
    if (incx == 1 && incy == 1) {
        key[0] = uplo; key[1] = 0;
    } else if (incy == 1) {
    ...
    PERF_EMIT("whpmv", key, N, iters, t_subject, t_mg);
