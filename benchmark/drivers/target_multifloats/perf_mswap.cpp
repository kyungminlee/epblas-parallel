...
static void run_mswap(int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        mswap_(&N, X, &one, Y, &one);          PERF_RESET(X, Xi, N, MFR); PERF_RESET(Y, Yi, N, MFR);
        mswap_migrated_(&N, X, &one, Y, &one); PERF_RESET(X, Xi, N, MFR); PERF_RESET(Y, Yi, N, MFR);
    }
    /* Per-call timing (reset out of the timed window — see PERF_TIME_PER_CALL). */
    double t_subject, t_mg;
    PERF_TIME_PER_CALL(t_subject, iters, PERF_RESET(X, Xi, N, MFR); PERF_RESET(Y, Yi, N, MFR), mswap_(&N, X, &one, Y, &one));
    PERF_TIME_PER_CALL(t_mg,      iters, PERF_RESET(X, Xi, N, MFR); PERF_RESET(Y, Yi, N, MFR), mswap_migrated_(&N, X, &one, Y, &one));
    PERF_EMIT("mswap", "-", N, iters, t_subject, t_mg);
