...
static void run_one(char side, char uplo, char trans, char diag,
    ...
    MFC *Bi = PERF_ALLOC(MFC, MNelt);
    PERF_FILL_C(MFC, A,  AAelt, 2);
    /* diagonal dominance for trsm */
    for (int i = 0; i < Asz; ++i) A[(size_t)i * lda + i] = Tc_from_d((double)(Asz + 4));
    PERF_FILL_C(MFC, Bi, MNelt, 4);
    PERF_RESET(B, Bi, MNelt, MFC);
    for (int r = 0; r < warmup; ++r) {
        wtrmm_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);          PERF_RESET(B, Bi, MNelt, MFC);
        wtrmm_migrated_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1); PERF_RESET(B, Bi, MNelt, MFC);
    }
    /* Per-call kernel-only timing — keep memcpy reset out of timed window. */
    double t_subject, t_mg;
    PERF_TIME_PER_CALL(t_subject, iters, PERF_RESET(B, Bi, MNelt, MFC),
        wtrmm_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1));
    PERF_TIME_PER_CALL(t_mg,      iters, PERF_RESET(B, Bi, MNelt, MFC),
        wtrmm_migrated_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1));
    char key[5] = {side, uplo, trans, diag, 0};
    PERF_EMIT("wtrmm", key, N, iters, t_subject, t_mg);
