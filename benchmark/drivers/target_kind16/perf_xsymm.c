...
static void run_one(char side, char uplo, int M, int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        xsymm_(&side, &uplo, &M, &N, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1);          PERF_RESET(C, Ci, MNelt, X16);
        xsymm_migrated_(&side, &uplo, &M, &N, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1); PERF_RESET(C, Ci, MNelt, X16);
    }
    double t_subject, t_mg;
    PERF_RESET(C, Ci, MNelt, X16);
    PERF_TIME(t_subject, iters, xsymm_(&side, &uplo, &M, &N, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1));
    PERF_RESET(C, Ci, MNelt, X16);
    PERF_TIME(t_mg,      iters, xsymm_migrated_(&side, &uplo, &M, &N, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1));
    char key[3] = {side, uplo, 0};
    PERF_EMIT("xsymm", key, N, iters, t_subject, t_mg);
