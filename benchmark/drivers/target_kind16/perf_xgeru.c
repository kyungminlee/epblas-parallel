...
static void run_one(int M, int N, int incx, int incy, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        xgeru_(&M, &N, &alpha, X, &incx, Y, &incy, A, &M);          PERF_RESET(A, Ai, MNelt, X16);
        xgeru_migrated_(&M, &N, &alpha, X, &incx, Y, &incy, A, &M); PERF_RESET(A, Ai, MNelt, X16);
    }
    /* Per-call timing (reset out of the timed window). */
    double t_subject, t_mg;
    PERF_TIME_PER_CALL(t_subject, iters, PERF_RESET(A, Ai, MNelt, X16), xgeru_(&M, &N, &alpha, X, &incx, Y, &incy, A, &M));
    PERF_TIME_PER_CALL(t_mg,      iters, PERF_RESET(A, Ai, MNelt, X16), xgeru_migrated_(&M, &N, &alpha, X, &incx, Y, &incy, A, &M));
    ...
    PERF_EMIT("xgeru", key, N, iters, t_subject, t_mg);
