...
static void run_one(char uplo, char ta, char tb, int N, int K, int iters, int warmup) {
    ...
    Q16 *A  = PERF_ALLOC(Q16, ABelt);
    Q16 *B  = PERF_ALLOC(Q16, BBelt);
    Q16 *C  = PERF_ALLOC(Q16, NNelt);
    Q16 *Ci = PERF_ALLOC(Q16, NNelt);
    PERF_FILL_R(Q16, A,  ABelt, 2);
    PERF_FILL_R(Q16, B,  BBelt, 3);
    PERF_FILL_R(Q16, Ci, NNelt, 4);
    PERF_RESET(C, Ci, NNelt, Q16);
    for (int r = 0; r < warmup; ++r) {
        qgemmtr_(&uplo, &ta, &tb, &N, &K, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1, 1);          PERF_RESET(C, Ci, NNelt, Q16);
        qgemmtr_migrated_(&uplo, &ta, &tb, &N, &K, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1, 1); PERF_RESET(C, Ci, NNelt, Q16);
    }
