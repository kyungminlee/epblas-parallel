...
static void run_one(char side, char uplo, char trans, char diag,
    ...
    Q16 *B  = PERF_ALLOC(Q16, MNelt);
    Q16 *Bi = PERF_ALLOC(Q16, MNelt);
    PERF_FILL_R(Q16, A,  AAelt, 2);
    /* diagonal dominance for trsm */
    for (int i = 0; i < Asz; ++i) A[(size_t)i * lda + i] = Tr_from_d((double)(Asz + 4));
    PERF_FILL_R(Q16, Bi, MNelt, 4);
    PERF_RESET(B, Bi, MNelt, Q16);
    for (int r = 0; r < warmup; ++r) {
        qtrsm_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);          PERF_RESET(B, Bi, MNelt, Q16);
        qtrsm_migrated_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1); PERF_RESET(B, Bi, MNelt, Q16);
    }
    /* Per-call kernel-only timing — keep memcpy reset out of timed window. */
    double t_subject, t_mg;
    PERF_TIME_PER_CALL(t_subject, iters, PERF_RESET(B, Bi, MNelt, Q16),
        qtrsm_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1));
    PERF_TIME_PER_CALL(t_mg,      iters, PERF_RESET(B, Bi, MNelt, Q16),
        qtrsm_migrated_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1));
    char key[5] = {side, uplo, trans, diag, 0};
    PERF_EMIT("qtrsm", key, N, iters, t_subject, t_mg);
