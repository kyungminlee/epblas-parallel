...
static void run_xswap(int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        xswap_(&N, X, &one, Y, &one);          PERF_RESET(X, Xi, N, X16); PERF_RESET(Y, Yi, N, X16);
        xswap_migrated_(&N, X, &one, Y, &one); PERF_RESET(X, Xi, N, X16); PERF_RESET(Y, Yi, N, X16);
    }
    /* Per-call timing (reset out of the timed window — see PERF_TIME_PER_CALL). */
    double t_subject, t_mg;
    PERF_TIME_PER_CALL(t_subject, iters, PERF_RESET(X, Xi, N, X16); PERF_RESET(Y, Yi, N, X16), xswap_(&N, X, &one, Y, &one));
    PERF_TIME_PER_CALL(t_mg,      iters, PERF_RESET(X, Xi, N, X16); PERF_RESET(Y, Yi, N, X16), xswap_migrated_(&N, X, &one, Y, &one));
    PERF_EMIT("xswap", "-", N, iters, t_subject, t_mg);
