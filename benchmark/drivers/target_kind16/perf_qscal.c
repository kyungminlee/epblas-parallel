...
static void run_qscal(int N, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        qscal_(&N, &alpha, X, &one);          PERF_RESET(X, Xi, N, Q16);
        qscal_migrated_(&N, &alpha, X, &one); PERF_RESET(X, Xi, N, Q16);
    }
    /* Per-call timing (reset out of the timed window). */
    double t_subject, t_mg;
    PERF_TIME_PER_CALL(t_subject, iters, PERF_RESET(X, Xi, N, Q16), qscal_(&N, &alpha, X, &one));
    PERF_TIME_PER_CALL(t_mg,      iters, PERF_RESET(X, Xi, N, Q16), qscal_migrated_(&N, &alpha, X, &one));
    PERF_EMIT("qscal", "-", N, iters, t_subject, t_mg);
    free(X); free(Xi);
}
