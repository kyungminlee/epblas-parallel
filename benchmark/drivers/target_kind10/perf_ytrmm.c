...
static void run_one(char side, char uplo, char trans, char diag,
    ...
    for (int r = 0; r < warmup; ++r) {
        ytrmm_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);          PERF_RESET(B, Bi, MNelt, C10);
        ytrmm_migrated_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1); PERF_RESET(B, Bi, MNelt, C10);
    }
    ...
    PERF_EMIT("ytrmm", key, N, iters, t_subject, t_mg);
