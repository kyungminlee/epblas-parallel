...
static void run_one(char uplo, int N, int K, int incx, int incy,
    ...
    for (int r = 0; r < warmup; ++r) {
        yhbmv_(&uplo, &N, &K, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1);          PERF_RESET(Y, Yi, leny, C10);
        yhbmv_migrated_(&uplo, &N, &K, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1); PERF_RESET(Y, Yi, leny, C10);
    }
    double t_subject, t_mg;
    PERF_RESET(Y, Yi, leny, C10);
    PERF_TIME(t_subject, iters, yhbmv_(&uplo, &N, &K, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1));
    PERF_RESET(Y, Yi, leny, C10);
    PERF_TIME(t_mg,      iters, yhbmv_migrated_(&uplo, &N, &K, &alpha, A, &LDA, X, &incx, &beta, Y, &incy, 1));
    char key[24];
    if (incx == 1 && incy == 1) {
        key[0] = uplo; key[1] = 0;
    } else if (incy == 1) {
    ...
    PERF_EMIT("yhbmv", key, N, iters, t_subject, t_mg);
