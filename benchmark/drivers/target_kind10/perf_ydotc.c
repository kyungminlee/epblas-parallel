...
static void run_one(int N, int iters, int warmup) {
    ...
    PERF_TIME(t_subject, iters, r = ydotc_(&N, X, &one, Y, &one); sink_T(&r));
    PERF_TIME(t_mg,      iters, r = ydotc_migrated_(&N, X, &one, Y, &one); sink_T(&r));
    PERF_EMIT("ydotc", "-", N, iters, t_subject, t_mg);
    free(X); free(Y);
}
