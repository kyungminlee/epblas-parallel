...
static void run_one(char side, char uplo, char trans, char diag,
    ...
    for (int r = 0; r < warmup; ++r) {
        etrmm_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1);          PERF_RESET(B, Bi, MNelt, R10);
        etrmm_migrated_(&side, &uplo, &trans, &diag, &M, &N, &alpha, A, &lda, B, &ldb, 1, 1, 1, 1); PERF_RESET(B, Bi, MNelt, R10);
    }
    ...
    PERF_EMIT("etrmm", key, N, iters, t_subject, t_mg);
