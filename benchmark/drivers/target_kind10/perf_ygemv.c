...
static void run_one(char trans, int M, int N, int incx, int incy,
    ...
    C10 *A  = PERF_ALLOC(C10, (size_t)M * N);
    C10 *X  = PERF_ALLOC(C10, lenx);
    C10 *Y  = PERF_ALLOC(C10, leny);
    C10 *Yi = PERF_ALLOC(C10, leny);
    PERF_FILL_C(C10, A,  (size_t)M * N, 2);
    PERF_FILL_C(C10, X,  lenx, 3);
    PERF_FILL_C(C10, Yi, leny, 4);

    PERF_RESET(Y, Yi, leny, C10);
    for (int r = 0; r < warmup; ++r) {
        ygemv_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1);          PERF_RESET(Y, Yi, leny, C10);
        ygemv_migrated_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1); PERF_RESET(Y, Yi, leny, C10);
    }

    double t_subject, t_mg;
    PERF_RESET(Y, Yi, leny, C10);
    PERF_TIME(t_subject, iters, ygemv_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1));
    PERF_RESET(Y, Yi, leny, C10);
    PERF_TIME(t_mg,      iters, ygemv_migrated_(&trans, &M, &N, &alpha, A, &M, X, &incx, &beta, Y, &incy, 1));
    ...
    PERF_EMIT("ygemv", key, N, iters, t_subject, t_mg);
