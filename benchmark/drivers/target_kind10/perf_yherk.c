...
static void run_one(char uplo, char trans, int N, int K, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        yherk_(&uplo, &trans, &N, &K, &alpha, A, &lda, &beta, C, &ldc, 1, 1);          PERF_RESET(C, Ci, NNelt, C10);
        yherk_migrated_(&uplo, &trans, &N, &K, &alpha, A, &lda, &beta, C, &ldc, 1, 1); PERF_RESET(C, Ci, NNelt, C10);
    }
    double t_subject, t_mg;
    PERF_RESET(C, Ci, NNelt, C10);
    PERF_TIME(t_subject, iters, yherk_(&uplo, &trans, &N, &K, &alpha, A, &lda, &beta, C, &ldc, 1, 1));
    PERF_RESET(C, Ci, NNelt, C10);
    PERF_TIME(t_mg,      iters, yherk_migrated_(&uplo, &trans, &N, &K, &alpha, A, &lda, &beta, C, &ldc, 1, 1));
    char key[3] = {uplo, trans, 0};
    PERF_EMIT("yherk", key, N, iters, t_subject, t_mg);
