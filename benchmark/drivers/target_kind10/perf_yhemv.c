...
static void run_one(char uplo, int N, int incx, int incy, int iters, int warmup) {
    ...
    for (int r = 0; r < warmup; ++r) {
        yhemv_(&uplo, &N, &alpha, A, &N, X, &incx, &beta, Y, &incy, 1);          PERF_RESET(Y, Yi, leny, C10);
        yhemv_migrated_(&uplo, &N, &alpha, A, &N, X, &incx, &beta, Y, &incy, 1); PERF_RESET(Y, Yi, leny, C10);
    }
    double t_subject, t_mg;
    PERF_RESET(Y, Yi, leny, C10);
    PERF_TIME(t_subject, iters, yhemv_(&uplo, &N, &alpha, A, &N, X, &incx, &beta, Y, &incy, 1));
    PERF_RESET(Y, Yi, leny, C10);
    PERF_TIME(t_mg,      iters, yhemv_migrated_(&uplo, &N, &alpha, A, &N, X, &incx, &beta, Y, &incy, 1));
    char key[24];
    if (incx == 1 && incy == 1) {
        key[0] = uplo; key[1] = 0;
    } else if (incy == 1) {
    ...
    PERF_EMIT("yhemv", key, N, iters, t_subject, t_mg);
