...
static void run_one(char uplo, char trans, int N, int K, int iters, int warmup) {
    ...
    R10 *B  = PERF_ALLOC(R10, AAelt);
    R10 *C  = PERF_ALLOC(R10, NNelt);
    R10 *Ci = PERF_ALLOC(R10, NNelt);
    PERF_FILL_R(R10, A,  AAelt, 2);
    PERF_FILL_R(R10, B,  AAelt, 3);
    PERF_FILL_R(R10, Ci, NNelt, 4);
    PERF_RESET(C, Ci, NNelt, R10);
    for (int r = 0; r < warmup; ++r) {
        esyr2k_(&uplo, &trans, &N, &K, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1);          PERF_RESET(C, Ci, NNelt, R10);
        esyr2k_migrated_(&uplo, &trans, &N, &K, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1); PERF_RESET(C, Ci, NNelt, R10);
    }
    double t_subject, t_mg;
    PERF_RESET(C, Ci, NNelt, R10);
    PERF_TIME(t_subject, iters, esyr2k_(&uplo, &trans, &N, &K, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1));
    PERF_RESET(C, Ci, NNelt, R10);
    PERF_TIME(t_mg,      iters, esyr2k_migrated_(&uplo, &trans, &N, &K, &alpha, A, &lda, B, &ldb, &beta, C, &ldc, 1, 1));
    char key[3] = {uplo, trans, 0};
    PERF_EMIT("esyr2k", key, N, iters, t_subject, t_mg);
