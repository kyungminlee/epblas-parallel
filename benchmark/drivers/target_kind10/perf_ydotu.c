...
static void run_one(int N, int iters, int warmup) {
    ...
    PERF_FILL_C(C10, Y, N, 1);
    for (int r2 = 0; r2 < warmup; ++r2) {
        r = ydotu_(&N, X, &one, Y, &one);          sink_T(&r);
        r = ydotu_migrated_(&N, X, &one, Y, &one); sink_T(&r);
    }
    double t_subject, t_mg;
    PERF_TIME(t_subject, iters, r = ydotu_(&N, X, &one, Y, &one); sink_T(&r));
    PERF_TIME(t_mg,      iters, r = ydotu_migrated_(&N, X, &one, Y, &one); sink_T(&r));
    PERF_EMIT("ydotu", "-", N, iters, t_subject, t_mg);
    free(X); free(Y);
}
