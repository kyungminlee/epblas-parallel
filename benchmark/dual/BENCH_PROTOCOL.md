...
trustworthy. The harness lives here (`benchmark/dual/`); generated drivers and raw
...
2. **Interleave per rep on shared buffers** (`benchmark/_perf_harness/dual.py`).
...
- `run_dual.sh <e|q|m> [routine,...]` — end-to-end: build the CMake archives
  ...
  (`benchmark/gen_dual_harnesses.py`), compile each against the ns archives +
...
- `benchmark/gen_dual_harnesses.py [--family e|q|m] [--routines a,b] [--list]` —
  emit the dual drivers (also driveable standalone).

The drivers and all raw `.omp{1,4}.txt` data stay in
`workspace/files/gap5/nsbench/` (gitignored); never under `benchmark/`.
